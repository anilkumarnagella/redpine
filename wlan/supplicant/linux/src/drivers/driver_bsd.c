...
bsd_set_if_media(void *priv, int media)
{
	...
	printf("IOCTL: %s %d media = %x\n", __func__, __LINE__, media);
...
bsd_set_mediaopt(void *priv, uint32_t mask, uint32_t mode)
{
    ...
    printf("Current media value : %x\n", media);
	if (media < 0)
		return -1;
	media &= ~mask;
	media |= mode;
    printf("************Setting media to:%x\n", media);
		      ...
		      const struct wpabuf *proberesp, const struct wpabuf *assocresp)
{
	...
	{
		printf("Failed to set IEEE80211_IOC_WPS ioctl\n");
		return -1;
	}
	if(beacon)
	{
		printf("Setting WPS IE FOR BEACON\n");
	...
	if(proberesp)
	{
		printf("Setting WPS IE FOR PROBE RESP\n");
...
wpa_driver_bsd_associate(void *priv, struct wpa_driver_associate_params *params)
{
	...
	if ((drv->capa.flags & WPA_DRIVER_FLAGS_P2P_CAPABLE) && (mode == IFM_IEEE80211_HOSTAP)) {
		...
		printf("change_vap_mode: status returned = %d\n", status);
...
wpa_driver_bsd_scan(void *priv, struct wpa_driver_scan_params *params)
{
	...
	{
		printf("####Setting WPS/P2P IE FOR PROBE REQ####\n");
  ...
  timeout = 5;
	...
	if(params->freqs)
	{
		...
		{
			printf("scanning selected channel freq = %d\n", params->freqs[count]);
		...
		switch(custom_cmd_type)
		{
			...
			case DRIVER_EVENT_MGMT:
			{
				...
				switch(WLAN_FC_GET_STYPE(rcv_pkt->frame_control))
				{
					...
					default:
					{
						printf("NON EXPECTING SUB TYPE = %0x", rcv_pkt->frame_control);
					}
					break;
				}
			}
			...
			default:
			{
				printf("Received unknown CUSTOM DRIVER_EVENT EVENT Value = %x\n", custom_cmd_type);
			}
			break;
		}
	}
...
{
	printf("before calling bsd_ctrl_iface in %s and %d\n",__func__,__LINE__);
