...
void p2p_set_probe_req_report(struct ieee80211vap *vap, uint16_t probe_req_report)
{
	//printk("P2P: In %s and %d\n", __func__, __LINE__);
	if(probe_req_report)
		printk("Starting probe req reporting\n");
	else
		printk("Stopping probe req reporting\n");
...
{ //CMD_CANCEL_REMAIN_ON_CHANNEL needs to be sent to supplicant
	printk("CMD_CANCEL_REMAIN_ON_CHANNEL came to driver from supplicant\n");
	//printk("P2P: In %s and %d\n", __func__, __LINE__);
...
static int change_vap_from_sta_to_hostap(struct ieee80211vap *vap)
{
	...
	//printk("in %s %d Resetting IFF_UP here\n", __func__, __LINE__);
...
static int p2p_change_vap_mode(struct ieee80211vap *vap, uint32_t mode)
{
	...
	printk("CMD_CHANGE_VAP_MODE came to supplicant from driver mode = %d\n", mode);
	//printk("vap->iv_opmode = %d mode = %d\n", vap->iv_opmode, mode);
	switch (mode)
	{
...
#define IEEE80211_GET_LE24(a) ((((u32) (a)[2]) << 16) | (((u32) (a)[1]) << 8) | \
	...
	switch (sub_type)
	{
		case IEEE80211_FC0_SUBTYPE_PROBE_REQ:
		{
			...
			{
				//printk("P2P: In %s and %d probe request reporting is not enabled\n", __func__, __LINE__);
				return;
			}
			...
			{ /* Send the probe response from here */
				...
				if(p2p == NULL)
				{
					//printk("P2P: In %s and %d Returning : no P2P ie\n", __func__, __LINE__);
					return;
				}
				//printk("P2P: In %s and %d  Verifying elements rates & xrates\n", __func__, __LINE__);
				IEEE80211_VERIFY_ELEMENT(rates, IEEE80211_RATE_MAXSIZE, return);
				if (xrates != NULL)
					IEEE80211_VERIFY_ELEMENT(xrates,
						IEEE80211_RATE_MAXSIZE - rates[1], return);
				//printk("P2P: In %s and %d  Verify element succeeded\n", __func__, __LINE__);
				if(is11b_rate_present(rates, xrates))
				{
					//printk("P2P: In %s and %d Returning : 11b rates present\n", __func__, __LINE__);
	//printk("In %s %d sub_type = %0x\n", __func__, __LINE__, sub_type);
					return;
				}
	...
	//printk("In %s %d sub_type = %0x\n", __func__, __LINE__, sub_type);
				...
				{
	//printk("In %s %d ssid_len = %d wild_ssid_len = %d\n", __func__, __LINE__, ssid[1], vap->p2p->p2p_wildcard_essid_len);
	//printk("In %s %d ssid = %s wild_ssid = %s\n", __func__, __LINE__, &ssid[2], vap->p2p->p2p_wildcard_essid);
					//printk("P2P: In %s and %d Returning : No P2P_WILDCARD_SSID\n", __func__, __LINE__);
					return;
				}
	...
	//printk("In %s %d sub_type = %0x\n", __func__, __LINE__, sub_type);
					//printk("P2P: In %s and %d Returning : Because of Hide SSID\n", __func__, __LINE__);
					return;
				}
				 ...
				 * the transmit side.
				 */
				//printk("In %s %d sending probe response\n", __func__, __LINE__);
...
void ieee80211_process_noa(struct ieee80211_p2p *p2p, uint8_t *frm, uint8_t *tstamp)
{
	...
	if (callout_pending(&p2p->noa_interval_timer) || 
		...
		printk("Quiet already in progress\n");
		return;
	}
	...
	if (p2p->noa == NULL) {
		...
		if (p2p->noa == NULL) {
			printk("In %s Line %d allocating memory for noa attr failed\n", __func__, __LINE__);
			return;
		}
		printk("In %s Line %d allocated memory for noa attr\n", __func__, __LINE__);
	}

	if (!p2p->noa->count) {
		memcpy(&p2p->noa->local_count, frm, 13);
		p2p->noa->count = p2p->noa->local_count;
		printk("Allocating values for first time\n");
	}
