...
uint16_t get_duration(uint16_t rate, uint16_t length, uint8_t ch_bw,uint16_t *spoof_len)
{
	...
	switch ((rate >> 7) & 0x3) {
		case DOT11AC_RATE:
			nltfs = 1;
			nes = 1; // will vary with ch bw and nss
			nss = 1;

			no_bits = (length << 3) + (16 + (6*nes));
			rate_val = rate & 0xF;

			no_sym_bits = ndbps_11ac[rate_val][ch_bw];
			no_sym_bits *= nss;
			no_symbols = (no_bits + no_sym_bits - 1) / no_sym_bits;

			if (rate & SHORT_GI_MODE) {
				no_symbols = HALF_GI_SYMBOL_TIME(no_symbols);
			}

			duration = no_symbols * 4;/* 4n sec */
			duration += PREAMBLE_11AC(nltfs);
			break;

		case DOT11N_RATE:
			//! find number of symbols: PLCP + data 
			rate_val = HT_RC_2_MCS(rate);
			no_bits = (length << 3) + OFDM_PLCP_BITS;

			if (ch_bw) {
				if (((rate_val > 20) && (rate_val < 24)) || (rate_val == 52)) {
					no_bits += NES_PLCP_BITS;
				}
			}

			no_sym_bits = bits_per_symbol[rate_val][ch_bw];
			no_symbols = (no_bits + no_sym_bits - 1) / no_sym_bits;

			nltfs = GET_NLTFS(rate);

			if (CHK_STBC_EN(rate)) {
				/**
				 * nltfs = nss + ness; ness = ntx - nsts; 
				 * nss == nsts if STBC = 0 (STBC = nsts - nss); nltfs = ntx 
				 */
				nltfs--;
				no_symbols += (no_symbols & BIT(0));
			}
			...
			//! addup duration for ht training and signal fields 

			//! add legacy training and signal fields for mixed mode 
			if (!(rate & GREENFIELD)) {
				duration += L_STF + L_LTF + L_SIG + HT_SIG + HT_STF + HT_LTF(nltfs); 
				*spoof_len = 3 * (no_symbols + nltfs + 3) - 3;
			} else {
				duration += HT_SIG + HT_GF_STF + HT_LTF1 + HT_LTF(nltfs);
				*spoof_len = 0;
			}
		...
		case DOT11G_RATE:
			...
			rate_val = rates_11g[((rate & 0xF) - 8)];

			no_sym_bits = (rate_val * OFDM_SYMBOL_TIME);
			no_bits = OFDM_PLCP_BITS + (length << 3);
			no_symbols = ((no_bits % no_sym_bits) == 0) ? 
				(no_bits / no_sym_bits) : ((no_bits / no_sym_bits) + 1);
			duration = OFDM_PREAMBLE_TIME + 
				(no_symbols * OFDM_SYMBOL_TIME);
			*spoof_len = 0;
			break;

		default:
			no_bits = length << 3;
			rate_val = rates_11b[(rate & 0xF)];

			if ((rate & 0xF) == 4) {
				no_bits = no_bits << 1;
			}

			duration = ((rate & SHORT_PREAMBLE) ? 
					((CCK_PREAMBLE_BITS + CCK_PLCP_BITS) >> 1) : 
					(CCK_PREAMBLE_BITS + CCK_PLCP_BITS)) + 
				(no_bits / rate_val);
			*spoof_len = 0;
			break;
	}
...
void channel_utilization(struct bpf_if *vap,int rssi_of_pkt,int length_of_pkt,int data_rate,char ch_bw,char crc)
{
	short len=0;
	struct ieee80211com *ic =NULL;
	unsigned long tot_on_air_occupancy;
	unsigned long tot_len_of_pkt;

  if(vap != NULL)
	  ic = vap->iv_ic;
	
	/*calculation of all the packets*/
	tot_on_air_occupancy =  get_duration(data_rate,length_of_pkt,ch_bw,&len);// cal on air time of packet
	ic->ch_util->tot_on_air_occupancy += tot_on_air_occupancy;// get_duration(data_rate,length_of_pkt,ch_bw,&len);// cal on air time of packet
	tot_len_of_pkt = length_of_pkt; // length of packets
	ic->ch_util->tot_length_of_pkt += tot_len_of_pkt; // length of packets
	ic->ch_util->tot_num_of_pkt += 1; // length of packets

	/*calculation of rssi packets*/
	if(ic->ch_util->rssi_threshold >= rssi_of_pkt )
	{	
		ic->ch_util->on_air_occupancy +=  tot_on_air_occupancy;//get_duration(data_rate,length_of_pkt,ch_bw,&len);// cal on air time of packet
		ic->ch_util->num_of_pkt += 1;// total number of packets
		if(crc)
		{
			ic->ch_util->crc_pkt += 1;
		}
		ic->ch_util->tot_len_of_pkt += tot_len_of_pkt; // length of packets
	}
}

void bpf_mtap2(struct bpf_if *vap, void *d,int l,struct mbuf *m)
{
  ...
  if(ch_num != vap->iv_ic->ic_curchan->ic_ieee)
  {
	...
	/* For Channel Utilization if set*/
	if(vap->iv_ic->ch_util->start_flag == 1)
	{
		channel_utilization(vap, rssi, skb->len, rate, band_width,crc);
	}
	netif_rx(skb);  
	kfree(m_copy);
	return;
...
void if_initname(struct ifnet *ifp,const char *name,int unit)
{
	...
	if (name != NULL)
	{
		...
		{
			printk("allocate name for VAP %s\n",name);
...
#endif
#endif
	printk("hard_start_xmit addr %p dev %p\n", ieee80211_hardstart, dev);
	return dev;
}
...
ifmedia_add(struct ifmedia *ifm, int mword, int data, void *aux)
{
	...
	entry->ifm_data = data;
	entry->ifm_aux = aux;
		//printk("In %s Adding ifm_media = %0x\n", __func__, entry->ifm_media);
