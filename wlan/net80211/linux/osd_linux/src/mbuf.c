                       ...
                       void *arg)
{
	m->m_flags |= M_TXCB;
#if  0
  struct ieee80211vap *vap = arg; /* FIXME */
...
struct netbuf_ctrl_block_m_s *netbuf_split(netbuf_ctrl_block_m_t *netbuf_cb, int len0, int wait)
{
	...
	/* TODO see if we need to reserve any head room here */
...
//TODO Take care of the ref count also
netbuf_ctrl_block_m_t *netbuf_copy (netbuf_ctrl_block_m_t *netbuf_cb, int malloc_flags)
{
	...
	memcpy(new_skb->cb, skb->cb, 48); //TODO Define this function once after netbuf_ctrl_block_m_t structure is freezed
