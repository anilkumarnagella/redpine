...
static void
onebox_completion_handler(PONEBOX_ADAPTER adapter,
                          netbuf_ctrl_block_t   *pkt )
{
	...
	if (pkt) 
	{
		...
		adapter->core_ops->onebox_core_radiotap_tx(adapter, vap, pkt);
	}
	return;
}
 ...
 */
static ONEBOX_STATUS prepare_onair_data_pkt_desc(PONEBOX_ADAPTER adapter, 
                                  ...
                                  int8 q_num)
{
	...
	struct chanAccParams *wme_params_sta;
		 ...
		 */
		adapter->os_intf_ops->onebox_change_hdr_size(netbuf_cb, (FRAME_DESC_SZ + sizeof(xtended_desc_t)));
		extnd_size = ((uint32 )netbuf_cb->data & 0x3); 
		adapter->os_intf_ops->onebox_change_hdr_size(netbuf_cb, extnd_size);
		adapter->os_intf_ops->onebox_memset(netbuf_cb->data,0,(FRAME_DESC_SZ + extnd_size + sizeof(xtended_desc_t)));
 ...
 /* Doing encap in firmware indicating in fd[1]*/
	...
	{
		//printk("indicating EOSP %s Line %d sta_id %d\n", __func__, __LINE__, netbuf_cb->sta_id);
...
//	frame_desc[6] |= ONEBOX_CPU_TO_LE16(key_idx << 13);
	...
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_DATA_SEND, (TEXT("Data pkt QUEUED \n")));
...
static ONEBOX_STATUS prepare_onair_mgmt_pkt_desc(PONEBOX_ADAPTER adapter, 
                                  netbuf_ctrl_block_t *netbuf_cb)
{
	uint16 *msg = NULL;
	int msg_len = 0;
	...
	if(ni == NULL) {
		printk("ERR: Freeing the skb as node(ni) has been freed\n");
		goto err;
	}
	vap = ni->ni_vap;
	...
	//FIXME:header size should be pulled with extended desc size
	if( (netbuf_cb->data[12] == 0x88) && (netbuf_cb->data[13] == 0x8e) && (netbuf_cb->flags & M_EAPOL) )
	{
		...
		if((((vap->iv_state == IEEE80211_S_RUN) 
			...
			printk("Change the Host_q to DATA_Q as it is rekeying\n");
			netbuf_cb->tid = 7;
			if(vap->iv_opmode == IEEE80211_M_STA)
			...
			status = prepare_onair_data_pkt_desc(adapter, netbuf_cb, netbuf_cb->priority);
			if(status != ONEBOX_STATUS_SUCCESS) {
				printk("ERR: Sending EAPOL is failed\n");
			}
			return status;
		}
	}
	adapter->os_intf_ops->onebox_change_hdr_size(netbuf_cb, (FRAME_DESC_SZ + sizeof(xtended_desc_t))); // + TX_VECTOR_SZ)); 
	extnd_size = ((uint32 )netbuf_cb->data & 0x3); 
	adapter->os_intf_ops->onebox_change_hdr_size(netbuf_cb, extnd_size); // + TX_VECTOR_SZ)); 
	adapter->os_intf_ops->onebox_memset(netbuf_cb->data,0,(FRAME_DESC_SZ + extnd_size + sizeof(xtended_desc_t)));// + TX_VECTOR_SZ));
	...
	if(!(netbuf_cb->flags & M_EAPOL))
	{
		...
		if(msg_len > (MAX_MGMT_PKT_SIZE)) 
		{
			...
			adapter->core_ops->onebox_dump(ONEBOX_ZONE_ERROR, (uint8 *)msg, msg_len);
			goto err;
		}
	}
	...
	if((vap->iv_state > IEEE80211_S_SCAN) && (vap->iv_opmode == IEEE80211_M_STA) 
			&&  !(ic->ic_flags & IEEE80211_F_SCAN) && (adapter->operating_band == BAND_2_4GHZ)) {
		for(ii = 0; jj< ni->ni_rates.rs_nrates; ii++) {
			if(ni->ni_rates.rs_rates[ii] & IEEE80211_RATE_BASIC) {
				for(jj = 0 ; jj < MAX_BG_SUPP_RATES; jj++) {
					if((adapter->std_rates[jj]) == (ni->ni_rates.rs_rates[ii] & IEEE80211_RATE_VAL)) {
						msg[4] = adapter->rps_rates[jj];
						break;
					}
				}
				break;
					...
					&& (netbuf_cb->flags & M_EAPOL))
	{
			...
			xtend_desc_pkt->retry_cnt = EAPOL_RETRY_CNT;
			printk("<==== Sending EAPOL PKT IN MGMT QUEUE ====>\n");
		}
#endif
	...
	{
		printk(" %s %d  Freeing  temp node in Hal\n", __func__, __LINE__);
		adapter->net80211_ops->onebox_free_node(ni);
	}

	return ONEBOX_STATUS_SUCCESS;
err:
	printk(" %s %d  Freeing skb in error case\n", __func__, __LINE__);
	adapter->os_intf_ops->onebox_free_pkt(adapter,netbuf_cb,0);
	return ONEBOX_STATUS_FAILURE;
}
/**
...
int core_xmit( PONEBOX_ADAPTER adapter, netbuf_ctrl_block_t *netbuf_cb )
{
	...
	if (!netbuf_cb->len)
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("%s:Zero Length packet\n"),__func__));
	...
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("%s: FSM state not open\n"),__func__));
		adapter->stats.tx_dropped++;
		goto xmit_fail;
	}
...
//	netbuf_cb->retry_count = 0;
	...
	if ((netbuf_cb->flags & ONEBOX_MULTICAST) || (netbuf_cb->flags & ONEBOX_BROADCAST))
	{
		if(vap->iv_opmode == IEEE80211_M_HOSTAP)
		{
			...
			if ((adapter->os_intf_ops->onebox_netbuf_queue_len(&core_vp->rv_mcastq)) > MULTICAST_WATER_MARK)
			{
				ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT(" MULTICAST Queue is Full\n")));
				goto xmit_fail;
			}
#ifndef BYPASS_TX_DATA_PATH
			...
			*(uint16_t *)((struct ieee80211_frame *)(netbuf_cb->data))->i_seq = ONEBOX_CPU_TO_LE16(seqno << IEEE80211_SEQ_SEQ_SHIFT);
			...
			//adapter->core_ops->onebox_dump(ONEBOX_ZONE_ERROR, netbuf_cb->data, 40);
			adapter->os_intf_ops->onebox_netbuf_queue_tail(&core_vp->rv_mcastq, netbuf_cb->pkt_addr);
			return ONEBOX_STATUS_SUCCESS;
		}
		else
		{
	...
	else
	{
		...
		adapter->stats.tx_bytes += netbuf_cb->len;
	}
	...
	/* Checking for WMM */
	if (adapter->vap_com.ic_flags & IEEE80211_F_WME) 
	{
	...
	{
		printk("In %s Dropping pkts as key is not installed opmode %d sta_id %d\n", 
			...
			((netbuf_cb->flags & M_EAPOL) && (netbuf_cb->data[12] == 0x88) && (netbuf_cb->data[13] == 0x8e)))
	{
		...
		printk("<==== EAPOLPKT priority changed to MGMT_QUEUE in core_xmit Line %d ====>\n" ,__LINE__);
	...
	{
		pkt_priority = MGMT_SOFT_Q;
		printk("<==== EAPOLPKT priority changed to MGMT_QUEUE in core_xmit ====>\n");
	}
	ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("Pkt Priority = %d\n"), pkt_priority));
	ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("Soft queue length before queueing %d\n"),
	adapter->os_intf_ops->onebox_netbuf_queue_len(&adapter->host_tx_queue[pkt_priority])));

	if((pkt_priority != MGMT_SOFT_Q ) && (pkt_priority != BROADCAST_HW_Q)) {
					/* Dropping Pkts as MAX QUEUE water mark level is reached */
					
					if((adapter->os_intf_ops->onebox_netbuf_queue_len(&adapter->host_tx_queue[pkt_priority]) + 1 ) >= adapter->host_txq_maxlen[pkt_priority]) {
	...
	if(vap->hal_priv_vap->stop_per_q[pkt_priority]) {
		...
		if ((vap->hal_priv_vap->conn_in_prog) && ((vap->iv_rsn_ie) || (vap->iv_wpa_ie))) {
...
#else
			adapter->os_intf_ops->onebox_stop_sub_txq(vap->iv_ifp, netbuf_cb->skb_priority);
#endif
	...
	if ( ((pkt_priority == VO_Q_STA) || (pkt_priority == VO_Q_AP)) && 
	     ((adapter->os_intf_ops->onebox_netbuf_queue_len(&adapter->host_tx_queue[pkt_priority]) + 1 ) >= VO_DATA_QUEUE_WATER_MARK )) {
			vap->hal_priv_vap->stop_udp_pkts[pkt_priority] = 1;
	}	else if (((pkt_priority == VI_Q_STA) || (pkt_priority == VI_Q_AP)) && 
	     ((adapter->os_intf_ops->onebox_netbuf_queue_len(&adapter->host_tx_queue[pkt_priority]) + 1 ) >= VI_DATA_QUEUE_WATER_MARK )) {
			vap->hal_priv_vap->stop_udp_pkts[pkt_priority] = 1;
	}	else if ( ((pkt_priority == BE_Q_STA) || (pkt_priority == BE_Q_AP)) && 
	     ((adapter->os_intf_ops->onebox_netbuf_queue_len(&adapter->host_tx_queue[pkt_priority]) + 1 ) >= BE_DATA_QUEUE_WATER_MARK ))	{
			vap->hal_priv_vap->stop_udp_pkts[pkt_priority] = 1;
	}	else if (((pkt_priority == BK_Q_STA)|| (pkt_priority == BK_Q_AP)) && 
	     ((adapter->os_intf_ops->onebox_netbuf_queue_len(&adapter->host_tx_queue[pkt_priority]) + 1 ) >= BK_DATA_QUEUE_WATER_MARK )) {
			vap->hal_priv_vap->stop_udp_pkts[pkt_priority] = 1;
	}
	...
	if(vap->hal_priv_vap->stop_udp_pkts[pkt_priority])
	{
		...
		ip = (struct iphdr *)&netbuf_cb->data[ snap_offset + sizeof(struct llc)]; /* FIXME yet to Handle the VLAN tagging case */
		//printk("ether type =%04x and ip proto=%04x\n",llc->llc_snap.ether_type, ip->protocol);
	...
	if(vap->hal_priv_vap->stop_udp_pkts[pkt_priority])
	{
		...
		/* Extract the llc and ip offsets */
		ip = (struct iphdr *)&netbuf_cb->data[ ETH_HDR_LEN]; /* FIXME yet to Handle the VLAN tagging case */
		if((netbuf_cb->data[ETH_PROTOCOL_OFFSET] == cpu_to_be16(0x0800)) && /* whether IP packet */
		   (ip->protocol == UDP_TYPE)) /* whether UDP packet */
		{
			adapter->udp_pkts_dropped++;
			if(adapter->udp_pkts_dropped && !(adapter->udp_pkts_dropped % 10000))
			{
				printk("no of udp_pkts_dropped = %d\n", adapter->udp_pkts_dropped);
			}
			goto xmit_fail;
	...
	*(uint16_t *)((struct ieee80211_frame *)(netbuf_cb->data))->i_seq = ONEBOX_CPU_TO_LE16(seqno << IEEE80211_SEQ_SEQ_SHIFT);
	...
	//printk("Identifier is %x\n",*(uint16_t *)&netbuf_cb->data[38]);
	//printk("Sequence number is %d\n", seqno);

	netbuf_cb->priority = (uint8)pkt_priority;
	//printk("%s %d: Queueing pkts to %d\n", __func__, __LINE__, pkt_priority);
	if((pkt_priority != MGMT_SOFT_Q)) {

			if(prepare_onair_data_pkt_desc(adapter, netbuf_cb, (pkt_priority))
							== ONEBOX_STATUS_SUCCESS){ 
					//printk("Queueing data pkts\n");
					core_queue_pkt(adapter, netbuf_cb, (pkt_priority));
	...
	} else if(prepare_onair_mgmt_pkt_desc(adapter, netbuf_cb) == ONEBOX_STATUS_SUCCESS) {
			printk("Queuing onair mgmt pkt\n");
			core_queue_pkt(adapter, netbuf_cb, (pkt_priority));
...
xmit_fail:	
	...
	adapter->total_tx_data_dropped[pkt_priority]++;
 ...
 * @return none.
 */
inline void core_queue_pkt(PONEBOX_ADAPTER adapter, 
                           netbuf_ctrl_block_t *netbuf_cb, uint8 q_num)
{
	...
	adapter->os_intf_ops->onebox_netbuf_queue_tail(&adapter->host_tx_queue[q_num], netbuf_cb->pkt_addr);
	return;
}
...
netbuf_ctrl_block_t* core_dequeue_pkt(PONEBOX_ADAPTER adapter, uint8 q_num)
{
	...
	{
		netbuf_cb = adapter->os_intf_ops->onebox_dequeue_pkt((void *)&adapter->host_tx_queue[q_num]);
	}
	return netbuf_cb;
}
...
int onebox_send_mgmt_pkt(struct ieee80211_node *ni, netbuf_ctrl_block_m_t *netbuf_cb_m, const struct ieee80211_bpf_params *bpf_params)
{
	...
	adapter = os_intf_ops->onebox_get_priv((void *)ic->ic_ifp);
	...
	if (!netbuf_cb->len)
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("%sZero Length mgmt packet\n"),__func__));
		goto send_mgmt_fail;
	}

	if (adapter->fsm_state != FSM_MAC_INIT_DONE)
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("%s: FSM state is not open \n"),__func__));
		goto send_mgmt_fail;
	}

	ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("%s: Mgmt Packet Length = %d\n"), __func__,netbuf_cb->len));

	netbuf_cb->ni = ni;

	if (adapter->os_intf_ops->onebox_netbuf_queue_len(&adapter->host_tx_queue[MGMT_SOFT_Q]) > 200) 
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("%s:Management Soft queue is full\n"),__func__));
		goto send_mgmt_fail;
	}
	...
	ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("%s: Adding to soft queue %d\n"),__func__, MGMT_SOFT_Q));
	if ((netbuf_cb->data[0] == 0x00) || (netbuf_cb->data[0] == 0x20)) 
	{
		if(vap->iv_opmode == IEEE80211_M_STA)
		{
			printk("Sending peer notify \n");
			core_send_station_info(ni, STA_CONNECTED);
		}
	}
	if(prepare_onair_mgmt_pkt_desc(adapter, netbuf_cb) == ONEBOX_STATUS_SUCCESS){
	core_queue_pkt(adapter, netbuf_cb, MGMT_SOFT_Q);

	/* Schedule the data transmit task */
	ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("Scheduling the qos_processor tasklet\n")));
	adapter->devdep_ops->onebox_schedule_pkt_for_tx(adapter);
