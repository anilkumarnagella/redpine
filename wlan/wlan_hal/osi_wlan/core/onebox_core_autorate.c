...
static void ar_rate_ctl_reset(PONEBOX_ADAPTER adapter, struct ieee80211_node *ni);
 ...
 * @return      none.
 */
static void *minstrel_alloc_sta(PONEBOX_ADAPTER adapter,struct ieee80211_node *ni)
{
	struct minstrel_node *mi;
	int max_rates = MAX_RATES; /* FIXME: Take from ni */

	/* Create minstrel node */
	mi = adapter->os_intf_ops->onebox_mem_zalloc(sizeof(struct minstrel_node), 0);
	if (!mi)
	{
		return NULL;
	}
	adapter->os_intf_ops->onebox_memset(mi, 0, sizeof(struct minstrel_node));

	/* Create Sample Table */
	mi->r = adapter->os_intf_ops->onebox_mem_zalloc(sizeof(struct minstrel_rate) * max_rates,0);
	if (!mi->r)
	{
		goto error;
	}
	adapter->os_intf_ops->onebox_memset(mi->r, 0, sizeof(struct minstrel_rate));

	mi->sample_table = adapter->os_intf_ops->onebox_mem_zalloc(SAMPLE_COLUMNS * max_rates, 0);
	if (!mi->sample_table)
	{
		goto error1;
	}

	mi->stats_update = adapter->os_intf_ops->onebox_get_jiffies();
	return mi;

error1:
	adapter->os_intf_ops->onebox_mem_free(mi->r);
error:
	adapter->os_intf_ops->onebox_mem_free(mi);
	return NULL;
}
...
{
	PONEBOX_ADAPTER adapter = (PONEBOX_ADAPTER)node->ni_vap->hal_priv_vap->hal_priv_ptr;
	struct minstrel_node *mn = NULL;

	mn = minstrel_alloc_sta(adapter, node);
 ...
 * @return      none.
 */
static void minstrel_free_sta(PONEBOX_ADAPTER adapter,struct ieee80211_node *sta)
{
	...
	{
		adapter->os_intf_ops->onebox_mem_free(mi->sample_table);
	}
	if (mi->r != NULL)
	{
		adapter->os_intf_ops->onebox_mem_free(mi->r);
...
{
	PONEBOX_ADAPTER adapter = (PONEBOX_ADAPTER)ni->ni_vap->hal_priv_vap->hal_priv_ptr;
	minstrel_free_sta(adapter, ni);
}
...
static void minstrel_update_stats(void *arg, struct ieee80211_node *ni)
{
	...
	PONEBOX_ADAPTER adapter = (PONEBOX_ADAPTER)arg;
	...
	if (!adapter->os_intf_ops->onebox_memcmp(ni->ni_vap->iv_myaddr,
	                                         ni->ni_macaddr,
	                                         ETHER_ADDR_LEN))
	{
		return;
	...
	sta_ratestats = adapter->autorate_stats[ni->hal_priv_node.sta_id];
	...
	adapter->core_ops->onebox_dump(ONEBOX_ZONE_AUTORATE, (uint8 *)sta_ratestats, 80);

	sn->stats_update = adapter->os_intf_ops->onebox_get_jiffies();
                         ...
                         struct ieee80211_tx_info *info)
{
	...
	//retry = max(2U, min(mr->retry_count_rtscts, retry)); TODO
	return retry;
}
 ...
 * @return      none.
 */
static void minstrel_get_rate(PONEBOX_ADAPTER adapter, struct ieee80211_node *ni, 
...
static void ar_newassoc(PONEBOX_ADAPTER sc, struct ieee80211_node *rn, int isnew)
{
	if (isnew)
	{
 ...
 * return       none.
 */
static void init_sample_table(PONEBOX_ADAPTER adapter,struct minstrel_node *mi,
                              struct ieee80211_node *ni)
{
	...
	mi->sample_column = 0;
	mi->sample_idx = 0;
	adapter->os_intf_ops->onebox_memset(mi->sample_table, 0, 
	...
	for (col = 0; col < SAMPLE_COLUMNS; col++) 
	{
		...
		{
			adapter->os_intf_ops->onebox_get_random_bytes(rnd, sizeof(rnd));
...
static int ieee80211_frame_duration(uint32 len, int32 rate, int32 erp, int32 short_preamble)
{
	...
	else 
	{
		 ...
		 */
		dur = 10; /* aSIFSTime = 10 usec */ /* FIXME: */
                                ...
                                struct ieee80211_rate *rate)
{
	...
	/* FIXME: multiply rate by 5, rate*10 is required input 
...
{
	uint32 i, n = 0;
	uint32 t_slot = 9; /* FIXME: get real slot time */
	...
	ni->ni_txrate = 0; /* txmit rate index */ /* FIXME: */
	...
	sn->sp_ack_dur = ieee80211_frame_duration(10,ctl_rate.bitrate*5,0,1); // FIXME
	...
	for (i = 0; i < sn->n_rates; i++) 
	{
		...
		uint32 cw = mp->cw_min;
		/* FIXME: Check for supported rates 
		 * if (!rate_supported(sta, sband->band, i))
		 * continue;
		 */

		n++;
		adapter->os_intf_ops->onebox_memset(mr, 0, sizeof(*mr));

		mr->rix = i;
		//mr->hix = i < ni->ni_rates.rs_nrates ? i : 12+ni->ni_htrates.rs_rates[i-ni->ni_rates.rs_nrates];
		ctrl_rate.flags = 0; /* FIXME:*/
		...
		//FIXME: Add a function in devdep layer
		//adapter->devdep_ops->onebox_get_hix(mr->bitrate);
		...
		switch(mr->bitrate)
		{
			...
			case IEEE80211_RATE_65M:
				...
				break;
			default:
				if(adapter->operating_band == BAND_2_4_GHZ)
				{
					mr->hix = 0;
				}
	...
	init_sample_table(adapter,sn, ni);
}
...
static void maxifi_rate_cb(void *arg, struct ieee80211_node *ni)
{
	...
	adapter = os_intf_ops->onebox_get_priv((void *)ni->ni_ic->ic_ifp);
	ar_rate_ctl_reset(adapter, ni);
}
...
static void ar_newstate(struct ieee80211vap *vap, enum ieee80211_state newstate)
{
	...
	if (newstate == IEEE80211_S_RUN) 
	{
		if (ic->ic_opmode != IEEE80211_M_STA) 
		{
			...
			adapter->net80211_ops->onebox_iterate_nodes(&ic->ic_sta, maxifi_rate_cb, NULL);
		}

		ar_newassoc(adapter, vap->iv_bss, 0);
	}
}
 ...
 * @return      none 
 */
static void *minstrel_alloc(PONEBOX_ADAPTER adapter)
{
	struct minstrel_priv *mp;

	mp = adapter->os_intf_ops->onebox_mem_zalloc(sizeof(struct minstrel_priv), 0);
	if (!mp)
	{
 ...
 * @return      none.
 */
static void minstrel_free(PONEBOX_ADAPTER adapter,void *priv)
{
	adapter->os_intf_ops->onebox_mem_free(priv);
}
 ...
 * @return      none.
 */
static void ar_detach(PONEBOX_ADAPTER adapter,struct wl_ratectrl *arc)
{
	struct minstrel_softc *osc = (struct minstrel_softc *) arc;

	if (ar_mp != NULL) 
	{
		minstrel_free(adapter,ar_mp);
	}
	if(osc == NULL)
	{
		return;
	...
	//FIXME: crashing while freeing this address FarSow
	// dynamic allocation was not not done for arc so can't use kfree here.
	//  adapter->os_intf_ops->onebox_mem_free(osc);
}
 ...
 * @return      none.
 */
static void ar_update_ratestats(PONEBOX_ADAPTER adapter)
{
	struct ieee80211com *ic = &adapter->vap_com;

	/* Sync rates for associated stations and neighbors. */
	adapter->net80211_ops->onebox_iterate_nodes(&ic->ic_sta, minstrel_update_stats, (void *)adapter);
}
...
struct minstrel_softc * ar_attach(PONEBOX_ADAPTER adapter,struct minstrel_softc *osc)
{

	struct minstrel_priv *mp = NULL; /* FreeBSD */

	osc = adapter->os_intf_ops->onebox_mem_zalloc(sizeof(struct minstrel_softc),0);
	if (osc == NULL) 
	{
	...
	mp = (struct minstrel_priv *) minstrel_alloc(adapter);
	ar_mp = mp;
 ...
 * @return      none.
 */
struct ieee80211_rate_ops *core_rate_attach(PONEBOX_ADAPTER adapter)
{
	struct minstrel_softc *min_sc=NULL;

	min_sc = ar_attach(adapter,min_sc);
 ...
 * @return      none.
 */
void core_rate_detach(PONEBOX_ADAPTER adapter)
{
	ar_detach(adapter,&rate_module);
	return;
}
