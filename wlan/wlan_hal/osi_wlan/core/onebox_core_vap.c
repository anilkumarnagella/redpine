...
void core_net80211_attach(PONEBOX_ADAPTER adapter)
{
	struct ieee80211com *ic = &adapter->vap_com;

	ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("\nCORE_MSG:Adding NET80211 functionality\n")));
	adapter->sc_nvaps = 0; //initially no vaps
	adapter->sc_default_ieee80211_debug = 0; //No debug option in VAP creation

	/*
	 * Setup rate tables for all potential media types.
	 */
	core_rate_setup(adapter, IEEE80211_MODE_11A);
	core_rate_setup(adapter, IEEE80211_MODE_11B);
	core_rate_setup(adapter, IEEE80211_MODE_11G);
	core_rate_setup(adapter, IEEE80211_MODE_11NA);
	core_rate_setup(adapter, IEEE80211_MODE_11NG);
  ...
  ic->ic_update_radio_params = core_radio_params_update;
	...
	ic->ic_headroom = (FRAME_DESC_SZ + TX_VECTOR_SZ);
	...
	/* Fill the RF type as the channel list needs to be populated based on the
 	 * 02/03 Modules 
 	 * */
	ic->ic_regdomain.pad[1] = adapter->operating_band;
	/* call MI attach routine. */
	adapter->net80211_ops->onebox_ifattach(ic,adapter->mac_addr);

	ic->ic_raw_xmit = onebox_send_mgmt_pkt;
	ic->ic_bsschan = &ic->ic_channels[0];
	adapter->beacon_interval = ONEBOX_CPU_TO_LE16(200); // FIXME
	ic->ic_bintval = adapter->beacon_interval;

	/* override default methods */
	adapter->sc_node_free = ic->ic_node_free;
	ic->ic_node_free = core_node_free;
	adapter->sc_node_cleanup = ic->ic_node_cleanup;
	ic->ic_node_cleanup = onebox_node_cleanup;
	adapter->sc_node_alloc  = ic->ic_node_alloc;
	ic->ic_node_alloc = core_node_alloc;
	ic->ch_util = adapter->os_intf_ops->onebox_mem_zalloc(sizeof(struct ch_utils),GFP_KERNEL);
	

	adapter->net80211_ops->onebox_radiotap_attach(ic, &adapter->txtap_hdr.wt_ihdr, 
	                                              sizeof(adapter->txtap_hdr),
	                                              0, &adapter->rxtap_hdr.wt_ihdr, 
	                                              sizeof(adapter->rxtap_hdr),0);
	adapter->init_net80211_done = 1;
	return;
}
...
ONEBOX_STATUS is_vap_valid(struct ieee80211vap *vap)
{
	...
	if (!vap) {
		...
		if (!vap->hal_priv_vap) {
			...
			ic = vap->iv_ic;
			os_intf_ops = onebox_get_os_intf_operations();
			...
			if(!(vap == adapter->hal_vap[vap_id].vap) || !(adapter->hal_vap[vap_id].vap_in_use)) {
				ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,
					     (TEXT("ERROR: In %s Line %d vap_id %d vap %p hal_vap %p vap_status %d\n "),
					     __func__, __LINE__, vap_id, vap, 
					     adapter->hal_vap[vap_id].vap, adapter->hal_vap[vap_id].vap_in_use));
				return ONEBOX_STATUS_FAILURE;
...
static ONEBOX_STATUS is_node_valid(PONEBOX_ADAPTER adapter, struct ieee80211_node *ni)
{
		uint32_t sta_index;

		if(!ni)
		{
				ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("ERROR: ni IS NULL In %s Line %d \n "),
										__func__, __LINE__));
				return ONEBOX_STATUS_FAILURE;
		...
		sta_index = ni->hal_priv_node.sta_id;
			 ...
			 * */
		if(sta_index >= adapter->max_stations_supported)
		{
				printk("ERROR: In %s Line %d Invalid Sta_id %d \n", __func__, __LINE__, sta_index);
				return ONEBOX_STATUS_FAILURE;
		}
			/* Already connected.
			 * Hence checking whether mac address matches and whether sta_connected_bitmap is set.
			 * */
		else if(!((!adapter->os_intf_ops->onebox_memcmp(adapter->sta[sta_index].mac_addr, ni->ni_macaddr, ETH_ALEN)) 
								&& (adapter->sta_connected_bitmap[sta_index /32] & BIT(sta_index %32))
						 )) {
				...
				onebox_print_mac_address(adapter, ni->ni_macaddr);
				ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("ERROR: The Mac adress in Hal sta data structure is \n ")));
				onebox_print_mac_address(adapter, adapter->sta[sta_index].mac_addr);
				return ONEBOX_STATUS_FAILURE;
		}
		return ONEBOX_STATUS_SUCCESS;
}
...
void onebox_scan_start(struct ieee80211com *ic)
{
	...
	printk("In %s line %d band_flags %02x\n", __func__, __LINE__, ic->band_flags);
	...
	{ 
					if ((vap->iv_opmode == IEEE80211_M_HOSTAP) 
									...
									if(!adapter->os_intf_ops->onebox_is_ifp_txq_stopped(vap->iv_ifp))
									{
													...
													if(send_vap_caps) {
																	printk("Send AP vap_caps updated\n");
																	vap->vap_caps(vap, 3); /*VAP UPDATE*/
													}
									}

									printk("Blocking AP data queues\n");
									adapter->block_ap_queues = 1;
									onebox_send_block_unblock(vap, STA_DISCONNECTED, 0);
									//break;
					}
					else if((vap->iv_opmode == IEEE80211_M_STA) && send_vap_caps) {
									printk("Send station vap_caps updated\n");
									vap->vap_caps(vap, 3); /*VAP UPDATE*/
					}

	}
	if(send_vap_caps) {
		printk("Sending bootup_params and radio_caps \n");
		adapter->operating_chwidth = BW_20Mhz;	
		onebox_load_bootup_params(adapter);
	 	adapter->endpoint_params.per_ch_bw = adapter->operating_chwidth;
	...
	printk("before Send Rx Filter frame to TA after scan start \n");	
	//adapter->rx_filter_word = 0; //ALLOWING ALL MGMT AND DATA
	status = onebox_send_rx_filter_frame(adapter, rx_filter_word);
	if(status < 0)
	{
		printk("Sending of RX filter frame on scan start failed\n");
	}
	return;
}
...
void onebox_scan_end(struct ieee80211com *ic)
{
	...
	{
		vap = adapter->hal_vap[adapter->sta_mode.vap_id].vap ;
		if(vap == NULL) {
			ONEBOX_DEBUG(ONEBOX_ZONE_ERROR, ("In %s line %d station VAP is NULL\n",__func__, __LINE__));
			dump_stack();
			return;
		}
		printk ("Send Rx Filter frame to TA after scan end \n");

		if (adapter->rx_filter_word) {
			  rx_filter_word = adapter->rx_filter_word;
		} else {
		...
		if (status < 0)
		    printk ("Sending of RX filter frame on scan end failed\n");

		printk("In %s Line %d Sending enable deep sleep\n", __func__, __LINE__);
...
static void p2p_hal_mode_change_from_sta_to_hostap(struct ieee80211vap *vap)
{
	...
	//vap->iv_flags |= IEEE80211_F_NOBRIDGE; //REVIEWME LATER TODO
                                       ...
                                       uint8_t macaddr[IEEE80211_ADDR_LEN])
{
	...
	switch (opmode) 
	{
		case IEEE80211_M_HOSTAP:
			...
			if ((adapter->sc_nvaps != 0) && (ic->ic_opmode == IEEE80211_M_STA))
			{
				return NULL;
			}
		...
		case IEEE80211_M_STA:
			if (adapter->sc_nstavaps != 0)  /* only one sta regardless */
			{
				return NULL;
			}
			if ((adapter->sc_nvaps != 0) && 
		...
		case IEEE80211_M_MONITOR:
			if (adapter->sc_nvaps != 0 && (ic->ic_opmode != opmode)) 
			{
	...
	if (adapter->sc_nvaps >= ONEBOX_VAPS_DEFAULT) 
	{
	...
	for (index = 0; index < ONEBOX_VAPS_DEFAULT; index++)
	{
		...
		base_mac_address = ((adapter->mac_addr[5] & 0xfc) + ((adapter->mac_addr[5] + index) & 0x03));
		fill = 1;
	...
	macaddr[5] = ((adapter->mac_addr[5] & 0xfc) + ((adapter->mac_addr[5] + free_id) & 0x03));
	vap_id = adapter->os_intf_ops->onebox_extract_vap_id(name);
#endif

	for(index = 0; index < ONEBOX_VAPS_DEFAULT; index++)
	{
		if(!adapter->hal_vap[index].vap_in_use){
				vap_id = index;
				break;
		}
	}
	...
	{
			printk("Unable to create VAP as MAX VAPS are already allocated\n");
			return NULL;

	}
	macaddr[5] = ((adapter->mac_addr[5] & 0xfc) | vap_id);
		
	vap = adapter->net80211_ops->onebox_vap_setup(ic, NULL, name, unit, opmode, flags, bssid , macaddr, 0);
	adapter->os_intf_ops->onebox_memcpy((PVOID)vap->iv_ifp->dev_addr, (const void *)macaddr, (int)6);
	vap->iv_rate = onebox_get_ieee80211_ratectl();
	...
	/*FIXME: Null for AP/other modes. Only station mode holds this */
	...
	vap->iv_flags_ht |= (IEEE80211_FHT_SHORTGI20 | IEEE80211_FHT_SHORTGI40 
                       | IEEE80211_FHT_STBC_RX | IEEE80211_FHT_RIFS  | IEEE80211_FHT_GF);
	...
	adapter->os_intf_ops->onebox_mem_alloc((PVOID *)&vap->hal_priv_vap, 
	                                       sizeof(struct hal_priv_ieee80211vap), GFP_ATOMIC);
	adapter->os_intf_ops->onebox_memset(vap->hal_priv_vap, 0, 
	                                    sizeof(struct hal_priv_ieee80211vap));
	adapter->os_intf_ops->onebox_mem_alloc((PVOID *)&vap->hal_priv_vap->core_vp, 
	                                        sizeof(struct core_vap), GFP_ATOMIC);
	adapter->os_intf_ops->onebox_memset(vap->hal_priv_vap->core_vp, 0, 
	                                    sizeof(struct core_vap));

	vap->hal_priv_vap->core_vp->av_newstate = vap->iv_newstate;
	vap->hal_priv_vap->hal_priv_ptr = (void *)adapter ;
#ifdef ONEBOX_CONFIG_CFG80211
	adapter->net80211_ops->onebox_cfg80211_attach(vap->iv_ifp, (void *)adapter->device, sizeof(struct ieee80211vap));
#endif
	vap->iv_newstate = onebox_newstate;
	...
	vap->iv_debug = 0x10C00000;
	if (adapter->sc_default_ieee80211_debug) 
	{
		/* User specified defaults for new VAPs were provided, so
		 * use those (only). */
		vap->iv_debug = (adapter->sc_default_ieee80211_debug & ~IEEE80211_MSG_DEBUG);
	}
	else 
	{
	...
	if(p2p_enable)
	{
		/* This function should be made part of net80211 callbacks */ //TODO
		adapter->net80211_ops->onebox_p2p_init(vap);
	...
	/* complete setup */
	status = adapter->net80211_ops->onebox_vap_attach(vap, adapter->net80211_ops->onebox_media_change,
	                                         adapter->net80211_ops->onebox_media_status, 0);
	if(status < 0){
			//FIMXE: Free the memory which is allocated for ifp and dev ptr
		printk("ERROR: Failed to create vap_interface \n");
		return NULL;
	}

	/** Storing Vap refernece for Hal*/
	adapter->hal_vap[vap_id].vap_in_use = 1;
	adapter->hal_vap[vap_id].vap = vap;
	...
	{
		adapter->sc_opmode = IEEE80211_M_HOSTAP;
	}
	else
	{
		/*
		...
		{
			adapter->sc_opmode = ic->ic_opmode;
		}
	}

	vap->hal_priv_vap->vap_id = vap_id;
 
	adapter->os_intf_ops->onebox_memcpy(adapter->std_rates, std_rates , sizeof(std_rates));
	adapter->os_intf_ops->onebox_memcpy(adapter->rps_rates, rps_rates , sizeof(rps_rates));
	if(opmode != IEEE80211_M_MONITOR) 	
	{
		printk("Sending vap_caps request vap_id %d\n", vap_id);
		adapter->devdep_ops->onebox_set_vap_capabilities(vap, VAP_ADD);
		//	adapter->devdep_ops->onebox_set_vap_capabilities(adapter, unit, opmode, NULL);
#ifdef BYPASS_TX_DATA_PATH
		if(opmode == IEEE80211_M_STA)
		{
			adapter->devdep_ops->onebox_send_block_unblock_data(vap, STA_DISCONNECTED, 0);
			adapter->sta_mode.vap_id = vap->hal_priv_vap->vap_id;
		}
#endif
	}
	if((adapter->sc_nstavaps) && (adapter->sc_nvaps > 1)) {
			TAILQ_FOREACH(vap_temp, &ic->ic_vaps, iv_next)
			{
					if(vap_temp->iv_opmode == IEEE80211_M_STA) {
							if(vap_temp->hal_priv_vap->ps_params.ps_en){
									printk("Disabling Power save as multi vap is enabled\n");
...
void onebox_vap_delete(struct ieee80211vap *vap, uint8 wait_for_lock)
{
	...
	printk("In %s Line %d\n", __func__, __LINE__);
	adapter->devdep_ops->onebox_set_vap_capabilities(vap, VAP_DELETE);
	printk("In %s Line %d\n", __func__, __LINE__);
#ifdef ENABLE_P2P_SUPPORT
	...
	if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
			for (sta_index=0; sta_index < adapter->max_stations_supported; sta_index++) {
					if ((adapter->sta_connected_bitmap[sta_index/32] & (1 << (sta_index%32)))) {//check for connected sta
							if (adapter->sta[sta_index].ni->ni_vap == vap) {//check whether sta maps to related vap 
									adapter->sta_connected_bitmap[sta_index /32] &= ~(1 << (sta_index % 32));
									adapter->os_intf_ops->onebox_memset(adapter->sta[sta_index].mac_addr, 0, ETH_ALEN);
									adapter->sta[sta_index].ni = NULL;
							}
					}
			}
	}
	...
	adapter->net80211_ops->onebox_vap_detach(vap, 0, wait_for_lock);
	if(stop_timer ){
			if(adapter->traffic_timer.function)
			{
					printk("<<<< Removing traffic timer >>>>\n");
	...
	adapter->hal_vap[vap_id].vap_in_use = 0;
	adapter->hal_vap[vap_id].vap = NULL;
	adapter->os_intf_ops->onebox_release_sem(&adapter->ic_lock_vap);

	if((adapter->sc_nstavaps) && (adapter->sc_nvaps < 2)) {
		TAILQ_FOREACH(vap_temp, &ic->ic_vaps, iv_next)
		{
			if(vap_temp->iv_opmode == IEEE80211_M_STA) {
				if(vap_temp->hal_priv_vap->ps_params.ps_en){
...
void onebox_send_sta_supported_features(struct ieee80211vap *vap, PONEBOX_ADAPTER adapter)
{
		...
		if(onebox_send_bgscan_params(vap, (uint16 *)&vap->hal_priv_vap->bgscan_params_ioctl, 0))
    {
      return;
		...
		send_bgscan_probe_req(adapter, vap->iv_bss, vap->hal_priv_vap->bgscan_params_ioctl.bg_cmd_flags);
	}
...
void onebox_newassoc(struct ieee80211_node *ni, int isnew)
{
	...
	uint8 i=0;
	//FIXME: vap id needs to be revisited
	...
	printk("#######################################\n");
	printk("IN %s Line %d  no of rates = %d no of ht rates = %d supported rates are\n", __func__, __LINE__,ni->ni_rates.rs_nrates,ni->ni_htrates.rs_nrates);
	for(i=0; i< ni->ni_rates.rs_nrates ; i++)
	{
		printk("%02x  ",ni->ni_rates.rs_rates[i]); 
	}
	printk("\n#######################################\n");


#ifdef AUTO_RATE_SUPPORT
	//adapter->core_rate_ops->newassoc(adapter, ni, 1); //Review me FIXME
	onebox_send_auto_rate_request(adapter, ni);
#endif
	if(vap->iv_opmode == IEEE80211_M_STA) {
		onebox_load_radio_caps(adapter);
...
struct ieee80211_node *core_node_alloc(struct ieee80211vap *vap, const uint8 *mac_addr)
{
	...
	os_intf_ops = onebox_get_os_intf_operations();
	adapter = os_intf_ops->onebox_get_priv(vap->iv_ic->ic_ifp);
	node = adapter->sc_node_alloc(vap, mac_addr);
...
void core_node_free(struct ieee80211_node *ni)
{
	...
	struct onebox_os_intf_operations *os_intf_ops;
	os_intf_ops = onebox_get_os_intf_operations();
	adapter = os_intf_ops->onebox_get_priv(ni->ni_ic->ic_ifp);

	adapter->sc_node_free(ni);
}
...
/**
  ...
  * @return void
  */
void core_set_channel(struct ieee80211com *ic)
{
	PONEBOX_ADAPTER adapter;
	...
	if(adapter->fsm_state == FSM_MAC_INIT_DONE)
	{
		...
		if (adapter->operating_band != rf_band)
		{
			adapter->operating_band = rf_band;
			adapter->rf_reset = 1;
			adapter->devdep_ops->onebox_program_bb_rf(adapter);
		}
#else
		adapter->endpoint_params.channel = ic->ic_curchan->ic_ieee;
		if(ic->band_flags & IEEE80211_CHAN_HT40) {
				printk("AP40: In %s Line %d 40Mhz***\n", __func__, __LINE__);
		...
		adapter->devdep_ops->onebox_band_check(adapter);
#endif		
		adapter->devdep_ops->onebox_set_channel(adapter, ic->ic_curchan->ic_ieee);
	}
}
...
int core_set_params(struct ieee80211com *ic, int cmd, int data)
{
	...
	switch (cmd)
	{
		case IEEE80211_IOCTL_SETMODE:  
			if (adapter->operating_band != data)
			{
				adapter->operating_band = data;
				adapter->rf_reset = 1;
				status = adapter->devdep_ops->onebox_program_bb_rf(adapter);
			}
		break;
		default:
...
int core_wme_update(struct ieee80211com *ic)
{
	...
	os_intf_ops = onebox_get_os_intf_operations();
	adapter = os_intf_ops->onebox_get_priv(ic->ic_ifp);
	//FIXME: status = adapter->devdep_ops->onebox_send_qos_conf_frame(adapter, ic->ic_wme.wme_wmeChanParams); 
	 ...
	 * or else care should be taken for hal_dequeue algo while setting these params*/ //FIXME

	/* Ideally while operating mltiple vaps, we should have dual params for both AP and STA 
	 * For now taking HOSTAP values only, even in case of STA */ //FIXME

	onebox_set_contention_vals(ic, adapter);
	status = onebox_load_radio_caps(adapter);
	return status;
}
...
void core_updateslot(struct ifnet *ifp)
{
	...
	adapter = os_intf_ops->onebox_get_priv(ifp);
	ic = &adapter->vap_com;
	TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next)
	{
	if( (vap != NULL) && (vap->iv_opmode == IEEE80211_M_STA) && ((vap->iv_state == IEEE80211_S_ASSOC) || (vap->iv_state == IEEE80211_S_RUN)))
	{
		...
		{
			printk("===> SHORT SLOT is enable <===\n");
			adapter->slot_rx_11n = SHORT_SLOT_VALUE;
		}
		else
		{
			printk("===> LONG SLOT is enable <===\n");
			adapter->slot_rx_11n = LONG_SLOT_VALUE;
		}
		if(ic->ic_flags & IEEE80211_F_SHPREAMBLE)
		{
			printk("===> SHORT PREAMBLE is enable <===\n");
			adapter->preamble_type = SHORT_PREAMBLE;
		}
		else
		{
			printk("===> LONG PREAMBLE is enable <===\n");
			adapter->preamble_type = LONG_PREAMBLE;
		}
		if(onebox_load_radio_caps(adapter) == ONEBOX_STATUS_SUCCESS)
		{
			ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,
...
int onebox_newstate(struct ieee80211vap *vap, enum ieee80211_state nstate, int arg)
{
	...
	ostate = vap->iv_state;
	os_intf_ops = onebox_get_os_intf_operations();
	adapter = os_intf_ops->onebox_get_priv(vap_com->ic_ifp);
	...
	if ((vap->iv_opmode == IEEE80211_M_HOSTAP) || (vap->iv_opmode == IEEE80211_M_STA))
	{
		 ...
		 */
		adapter->core_rate_ops->newstate(vap, nstate);
		     ...
		     (vap->iv_opmode == IEEE80211_M_HOSTAP)) 
		{
			...
			ni = vap->iv_bss;
			bo = &core_vp->rv_boff;
			core_vp->rv_bcbuf = adapter->net80211_ops->onebox_beacon_alloc(ni, bo);
			if (core_vp->rv_bcbuf == NULL) 
			{
				return ONEBOX_STATUS_FAILURE;
			}
			adapter->os_intf_ops->onebox_netbuf_queue_init(&core_vp->rv_mcastq);
#if 0
			/* Giving Beacon Slot to the VAP */
			for (ii = 0; ii < ONEBOX_VAPS_DEFAULT; ii++) 
			{
				if (adapter->hal_vap[ii].vap == NULL) 
				{
					adapter->hal_vap[ii].vap = vap;
					vap->hal_priv_vap->beacon_loc = ii;
					break;
				}
			}
				if (adapter->hal_vap[vap->hal_priv_vap->vap_id].vap == NULL) 
				{
					adapter->hal_vap[vap->hal_priv_vap->vap_id].vap = vap;
					vap->hal_priv_vap->beacon_loc = vap->hal_priv_vap->vap_id;
				}
#endif

			/* Set AMPDU Density */
			adapter->mpdu_density = vap->iv_ampdu_density;
			ONEBOX_DEBUG(ONEBOX_ZONE_INFO,("CORE_MSG: MPDU Density = %d\n", adapter->mpdu_density));
		...
		else if ((vap->hal_priv_vap->beacon_loc == 100) && (nstate == IEEE80211_S_SCAN) && 
		...
		if (nstate == IEEE80211_S_INIT)
		{
			...
			{
				adapter->hal_vap[vap->hal_priv_vap->beacon_loc].vap = NULL;
...
int core_rate_setup(PONEBOX_ADAPTER adapter, uint8 mode)
{
	struct ieee80211com *ic = &adapter->vap_com;
	...
	switch (mode) 
	{
		...
		case IEEE80211_MODE_11NA:
			...
			adapter->os_intf_ops->onebox_memset(ic->ic_sup_rates[mode].rs_rates, 
			                                    0, ic->ic_sup_rates[mode].rs_nrates);
			adapter->os_intf_ops->onebox_memcpy(ic->ic_sup_rates[mode].rs_rates, 
			                                    &rs_rates[4], ic->ic_sup_rates[mode].rs_nrates);
			break;
		case IEEE80211_MODE_11B:
			ic->ic_sup_rates[mode].rs_nrates = 4;
			adapter->os_intf_ops->onebox_memset(ic->ic_sup_rates[mode].rs_rates, 0, 
	...
	adapter->net80211_ops->onebox_setbasicrates(&ic->ic_sup_rates[mode], mode);
	return 1;
}
 ...
 */ 
void vap_load_beacon(PONEBOX_ADAPTER adapter, int32 vap_id)
{
	struct ieee80211vap *vap = NULL;
	struct core_vap *core_vp;
	...
	struct ieee80211com *ic= &adapter->vap_com;
	int mcast_len;
	vap = adapter->hal_vap[vap_id].vap;
	if (vap == NULL)
	{
	...
	mcast_len=  adapter->os_intf_ops->onebox_netbuf_queue_len(&core_vp->rv_mcastq);
	...
	if (adapter->net80211_ops->onebox_beacon_update(ni, bo, netbuf_cb_m, mcast_len)) 
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,("\nCORE_MSG: There is a change in beacon payload length"));
	}

	//netbuf_cb = onebox_translate_mbuf_to_netbuf(netbuf_cb_m);
	netbuf_cb = adapter->wlan_osd_ops->onebox_get_skb_cb(netbuf_cb_m);
	...
	if (adapter->fsm_state == FSM_MAC_INIT_DONE) 
	{
		core_send_beacon(adapter, netbuf_cb, core_vp);
	}
	return;
}
...
int core_send_station_info(struct ieee80211_node *ni, int notify_event)
{
	...
	if(notify_event == STA_CONNECTED)
	{
		...
		for (sta_index = 0; sta_index < adapter->max_stations_supported; sta_index++)
		{
			if(!adapter->os_intf_ops->onebox_memcmp(adapter->sta[sta_index].mac_addr, ni->ni_macaddr, ETH_ALEN))
			{
		...
		if(sta_index == adapter->max_stations_supported) /* In case if not already connected*/
		{
			...
			for (sta_index = 0; sta_index < adapter->max_stations_supported; sta_index++)
			{
				if (!(adapter->sta_connected_bitmap[sta_index/32] & (1 << (sta_index%32))))
				{
					adapter->os_intf_ops->onebox_memcpy(adapter->sta[sta_index].mac_addr, ni->ni_macaddr, ETH_ALEN);
					adapter->sta_connected_bitmap[sta_index /32] |= (1 << (sta_index % 32));
		...
		if (sta_index == adapter->max_stations_supported) /* Reached max supported stations */
		{
			return ONEBOX_STATUS_FAILURE;
		}
	}
		...
		sta_index = ni->hal_priv_node.sta_id;
		if(is_node_valid(adapter, ni) < 0) {
			ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,
					(TEXT("ERROR: In %s Line %d The sta_id in Hal_station data structure and ni doesn't matches ni_sta_id %d hal_sta id %d\n"), 
					 __func__, __LINE__, sta_index, adapter->sta[sta_index].sta_id));
			dump_stack();
			return ONEBOX_STATUS_FAILURE;
		}
	...
	sta = &(adapter->sta[ni->hal_priv_node.sta_id]); 
	...
	if(notify_event == STA_CONNECTED)
	{
		...
		if(vap->iv_opmode == IEEE80211_M_STA) {
				adapter->sta_mode.sta_id = ni->hal_priv_node.sta_id;
				adapter->sta_mode.vap_id = vap->hal_priv_vap->vap_id;
		}
	}
	else if(notify_event == STA_DISCONNECTED)
	...
	status = adapter->devdep_ops->onebox_hal_send_sta_notify_frame(adapter, ni, notify_event);
	if (status) 
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("\nCORE_MSG:Sending STA notification failed")));
		return ONEBOX_STATUS_FAILURE;
	}
/*FIXME: IN case if there is a band change then BB/RF programming  will
 ...
 * By checking this state we will move to fsm_mac_init_done */
	...
	{
#ifdef AUTO_RATE_SUPPORT
		adapter->core_rate_ops->newassoc(adapter, ni, 1);
#endif
		printk("40Mhz: In %s Line %d ni->ni_chan.ic_flags = %02x ic->ic_curchan->ic_flags = %02x\n", __func__, __LINE__, ni->ni_chan->ic_flags, ic->ic_curchan->ic_flags);
		if(ic->band_flags & IEEE80211_CHAN_HT40)
		{
			printk("In %s %d calling chwidth change here \n", __func__, __LINE__);
			/* After Assoc ch width changed to 40Mhz */
#ifndef PROGRAMMING_BBP_TA
			adapter->operating_chwidth = BW_40Mhz;
			printk("In %s Line %d operating chwidth = %d def_chwidth = %d\n", __func__, __LINE__,adapter->operating_chwidth,adapter->def_chwidth);
			//if(adapter->operating_chwidth != adapter->def_chwidth)
			{
				...
				printk("Enabling band change and program bbrf\n");
				adapter->chw_flag = 1;
				adapter->devdep_ops->onebox_program_bb_rf(adapter);
				adapter->def_chwidth = adapter->operating_chwidth;
			}
#else
			adapter->endpoint_params.channel = ic->ic_curchan->ic_ieee;
			adapter->endpoint_params.per_ch_bw = BW_40Mhz;
			adapter->devdep_ops->onebox_band_check(adapter);
			printk("In %s Line %d operating chwidth = %d def_chwidth = %d\n", __func__, __LINE__,adapter->endpoint_params.per_ch_bw,adapter->def_chwidth);
			adapter->def_chwidth = adapter->operating_chwidth;
#endif			
			{
				/*40mhz connection */
				printk("%dSetting channel = %d ic_flags =%x\n", __LINE__, ic->ic_curchan->ic_ieee, ic->ic_curchan->ic_flags);
				if(ic->band_flags & IEEE80211_CHAN_HT40U)
				{
					/*secondary channel is above the primary channel */
					printk("40mhz: Secondary channel is above the primary channel\n");
				...
				else if(ic->band_flags & IEEE80211_CHAN_HT40D)
				{
					...
					printk("40mhz: Secondary channel is below the primary channel\n");
					sec_chan = (ic->ic_curchan->ic_ieee - 4);
					center_freq = ((sec_chan + ic->ic_curchan->ic_ieee)/2);
				}
			}
			printk("%dSecondary channel = %d cent_freq=%d\n", __LINE__, sec_chan, center_freq);
			printk("In %s Line %d operating chwidth = %d cur chan=%d\n", __func__, __LINE__,adapter->operating_chwidth, ic->ic_curchan->ic_ieee);
			adapter->devdep_ops->onebox_set_channel(adapter, center_freq);
		}
	}
	else if ((vap->iv_opmode == IEEE80211_M_STA) && (notify_event == STA_DISCONNECTED)) 
	{
		/*FIXME: Clearing this flag because HT40 flag is set in HTParsing code eventhough AP is not supporting 40mhz
		...
		{
#ifndef PROGRAMMING_BBP_TA
			adapter->operating_chwidth = adapter->def_chwidth = BW_20Mhz;
			printk("In %s Line %d operating chwidth = %d def_chwidth = %d\n", __func__, __LINE__,adapter->operating_chwidth,adapter->def_chwidth);
			if(onebox_load_bootup_params(adapter) == ONEBOX_STATUS_SUCCESS)
			{
				ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,
...
#else
			adapter->endpoint_params.channel = ic->ic_curchan->ic_ieee;
			adapter->endpoint_params.per_ch_bw = BW_20Mhz;
			adapter->devdep_ops->onebox_band_check(adapter);
			printk("In %s Line %d operating chwidth = %d def_chwidth = %d\n", __func__, __LINE__,adapter->endpoint_params.per_ch_bw,adapter->def_chwidth);
			adapter->def_chwidth = adapter->operating_chwidth;
			adapter->devdep_ops->onebox_set_channel(adapter, ic->ic_curchan->ic_ieee);
                     ...
                     ieee80211_keyix *rxkeyix)
{
	 ...
	 */
	adapter->sc_mcastkey = 0;
	if ((k->wk_flags & IEEE80211_KEY_GROUP) && !adapter->sc_mcastkey) 
	{
		int32 i;
...
static int onebox_hal_keyset(PONEBOX_ADAPTER adapter, 
                      const struct ieee80211_key *k,
                      ...
                      uint32 cipher)
{
	...
	{
		adapter->os_intf_ops->onebox_memcpy(key_buf,k->wk_key,key_len);
		adapter->core_ops->onebox_dump(ONEBOX_ZONE_ERROR, key_buf, key_len);
		if(cipher == IEEE80211_CIPHER_TKIP)
		{
			key_len += IEEE80211_MICBUF_SIZE;
			adapter->os_intf_ops->onebox_memcpy(&key_buf[16],&k->wk_key[16],IEEE80211_MICBUF_SIZE);
		}
	...
	{  
		...
		adapter->os_intf_ops->onebox_memcpy(&key_buf[ix],k->wk_key,key_len);
		if(cipher == IEEE80211_CIPHER_TKIP)
		{
			key_len += IEEE80211_MICBUF_SIZE;
		}
	...
	{
		if (vap->iv_opmode == IEEE80211_M_STA) {
			node_id = adapter->sta_mode.sta_id;
		} else {
			node_id = vap_id;
		}
	}
	adapter->sec_mode[vap_id] = cipher;

	if(k->wk_flags & IEEE80211_KEY_XMIT)
	{
		printk("In %s and %d \n", __func__, __LINE__);
    if(cipher == IEEE80211_CIPHER_WEP) 
      adapter->wep_key_idx[vap_id]  = k->wk_keyix;
	...
	if(cipher == IEEE80211_CIPHER_WEP)
	{
		...
		{
			printk("In %s and %d \n", __func__, __LINE__);
			adapter->devdep_ops->onebox_hal_load_key(adapter, key_buf, key_len, node_id, key_type, k->wk_keyix, cipher, vap);
		}
	  adapter->wep_keylen[vap_id][k->wk_keyix] = key_len;
		adapter->os_intf_ops->onebox_memcpy(&adapter->wep_key[vap_id][0][k->wk_keyix][0],key_buf,key_len);
		adapter->core_ops->onebox_dump(ONEBOX_ZONE_DEBUG, (PUCHAR)&adapter->wep_key, 128);
		return 0;
	}
	ret = adapter->devdep_ops->onebox_hal_load_key(adapter, key_buf, key_len, node_id, key_type,k->wk_keyix, cipher, vap);
	if(key_type == ONEBOX_PAIRWISE_KEY)
	{
		if(!ret)
		{
			printk("Enable encryption as Pair wise key loading is successfull, ni_flags %x \n", ni_sta->ni_flags);
...
#define N(a)    ((int)(sizeof(a)/sizeof(a[0])))
	static const u_int8_t ciphermap[] = { HAL_CIPHER_WEP,      /* IEEE80211_CIPHER_WEP */
	...
	adapter->os_intf_ops->onebox_memset(&hk, 0, sizeof(hk));
	/*
	...
	if ((k->wk_flags & IEEE80211_KEY_SWCRYPT) == 0)
	{
		...
		adapter->os_intf_ops->onebox_memcpy(hk.kv_val, k->wk_key, k->wk_keylen);
	}
	else
	{
	...
	if ((k->wk_flags & IEEE80211_KEY_GROUP) && adapter->sc_mcastkey)
	{
		 ...
		 */
		printk("ERROR: Group key and sc_mcast key installed %02x %02x %02x %02x %02x %02x\n",
						bss->ni_macaddr[0],
						bss->ni_macaddr[1],
						bss->ni_macaddr[2],
						bss->ni_macaddr[3],
	...
	ni = adapter->net80211_ops->onebox_find_node_locked_debug(nt, mac, func, line);
#else
	ni = adapter->net80211_ops->onebox_find_node_locked(nt, mac);
#endif
	...
	if((is_node_valid(adapter, ni) < 0) && !((mac0[0] == 0xff) && (mac0[1] == 0xff) && (mac0[5] == 0xff))) {
		...
		onebox_print_mac_address(adapter, (uint8_t *)mac0);
		dump_stack();
		return ONEBOX_STATUS_FAILURE;
	}
	...
	return onebox_hal_keyset(adapter, k, vap, ni, 
 ...
 * @return void
 */
void core_net80211_detach(PONEBOX_ADAPTER adapter)
{
	struct ieee80211com *ic = &adapter->vap_com;
	adapter->os_intf_ops->onebox_mem_free(ic->ch_util);
	if(adapter->init_net80211_done == 1) {
		adapter->net80211_ops->onebox_ifdetach(ic);
	adapter->init_net80211_done = 0;
	}
}
...
{
	ic->ic_regdomain.country = 840;
	ic->ic_regdomain.location = ' ';
	ic->ic_regdomain.isocc[0] = '\0';
	ic->ic_regdomain.isocc[1] = '\0';
	ic->ic_regdomain.pad[0] = 3;			//Just an indication of the country, to refer index in regdm_table[]
}

ONEBOX_STATUS onebox_send_auto_rate_request (PONEBOX_ADAPTER adapter, struct ieee80211_node *ni )
{
	onebox_mac_frame_t *mgmt_frame;
...
//	uint16 mcs_1[] = {13, 26, 39, 52, 78, 104, 117, 130};
	...
	adapter->auto_rate_params.failure_limit      = 3; 
	adapter->auto_rate_params.Initial_boundary   = 3;
	adapter->auto_rate_params.max_threshold_limt = 27;
	...
	adapter->os_intf_ops->onebox_memcpy((PVOID)&mgmt_frame->u.auto_rate.failure_limit,
						(const void *)&adapter->auto_rate_params,
	...
	if(ic->band_flags & IEEE80211_CHAN_HT40) {
					...
					{
									printk("AUT0_RATE: Node is capable of 40Mhz\n");
	...
	printk("In %s Line %d Supported rates = %d \n", __func__, __LINE__, rs.rs_nrates);	
	adapter->core_ops->onebox_dump(ONEBOX_ZONE_ERROR, (PUCHAR)rs.rs_rates, rs.rs_nrates);

	printk("Supported ht rates ni %s:%d\n", __func__, __LINE__);	
	adapter->core_ops->onebox_dump(ONEBOX_ZONE_ERROR, (PUCHAR)htrs.rs_rates, htrs.rs_nrates);
  ...
  if (htrs.rs_nrates) {
	...
	printk("rate bitmap is %d\n", ht_rate_bitmap);

	if(rs.rs_nrates == 8)	/*station is 11g supported*/
	{
		min_rate = RSI_RATE_6;
	}
	else if(rs.rs_nrates == 0)	/*station is 11n only supported*/
	{
		min_rate = RSI_RATE_MCS0;
	}
	else if(adapter->operating_band == BAND_5GHZ)			
	{
		min_rate = RSI_RATE_6;	
	}
	else	/*station is 11bgn only supported*/
	{
		min_rate = RSI_RATE_1;	
	}
	...
	//if(adapter->operating_band == BAND_5GHZ)
	{
		...
		printk("<=== Supported rates after masking the 8th bit ===>\n");
		adapter->core_ops->onebox_dump(ONEBOX_ZONE_ERROR, (PUCHAR)rs.rs_rates, rs.rs_nrates);
	}
	...
	if(htrs.rs_nrates && rs.rs_nrates)/** BG/N Mode **/
	{
		ii = rs.rs_nrates - 1;
		jj = htrs.rs_nrates - 1;
		printk("<===== Connected in A/BG/N Mode =====>\n");
		for( kk = 0; kk < (rs.rs_nrates + htrs.rs_nrates); kk++)
		{
			//printk("ii = %d jj = %d kk = %d rs.rs_rates[%d] = %d mcs [%d] = %d\n", ii, jj, kk, ii, rs.rs_rates[ii], jj, mcs[jj]);
			if(rs.rs_rates[ii] < mcs[jj])
			{
				//printk("copying final from map_rates ii = %d jj = %d kk = %d rs.rs_rates[%d] = %d \n", ii, jj, kk, ii, rs.rs_rates[ii]);
				...
				if(jj == 0)
				{
					...
					for(; kk < (rs.rs_nrates + htrs.rs_nrates); kk++)
					{
													...
													(rs.rs_rates[ii] == (STD_RATE_01)))
						{
							...
							{
								if((adapter->std_rates[p]) == rs.rs_rates[ii])
								{
									mgmt_frame->u.auto_rate.supported_rates[kk] = adapter->rps_rates[p];
									ii--;
									break;
								}
			...
			else
			{
											...
											(rs.rs_rates[ii] == (STD_RATE_01)))
				{
					for(p =0 ; p < MAX_BG_SUPP_RATES; p++)
					{
						//if((adapter->std_rates[p] & 0x7f) == rs.rs_rates[ii])
						if((adapter->std_rates[p]) == rs.rs_rates[ii])
						{
							mgmt_frame->u.auto_rate.supported_rates[kk] = adapter->rps_rates[p];
						}
	...
	{
		printk("<====== Connected in BG Mode  Band = %d =========>\n", adapter->operating_band);
		//ii = (rs.rs_nrates - 1);
		//for(kk = 0; kk < rs.rs_nrates ; kk++)
		{
			for(ii = 12; ii >= 0 ; ii--)
			{
						printk("rs.rs_rates[%d] = %02x \n", ii, rs.rs_rates[ii] );
						if( (rs.rs_rates[ii] == (STD_RATE_54)) ||
													(rs.rs_rates[ii] == (STD_RATE_48)) ||
													(rs.rs_rates[ii] == (STD_RATE_36)) ||
													(rs.rs_rates[ii] == (STD_RATE_24)) ||
													...
													(rs.rs_rates[ii] == (STD_RATE_01)))
						{
							for(p = 0 ; p < MAX_BG_SUPP_RATES ; p++)
							{
								if((adapter->std_rates[p]) == rs.rs_rates[ii])
								{
									//printk("rs.rs_rates[%d] = %02x std_rates[%d] = %02x\n", ii,rs.rs_rates[ii], p, adapter->std_rates[p]);
									mgmt_frame->u.auto_rate.supported_rates[kk] = adapter->rps_rates[p];
									//printk("mgmt_frame->u.auto_rate.supported_rates[%d] = %02x \n", kk, mgmt_frame->u.auto_rate.supported_rates[kk]);
									kk++;
								}
							}
						}
	...
	printk("Auto rate frame dump\n");
	adapter->core_ops->onebox_dump(ONEBOX_ZONE_ERROR, (PUCHAR)mgmt_frame, (FRAME_DESC_SZ + sizeof(mgmt_frame->u.auto_rate) + (2*mgmt_frame->u.auto_rate.num_supported_rates)));
	status = adapter->devdep_ops->onebox_send_internal_mgmt_frame(adapter, (uint16 *)mgmt_frame,  (FRAME_DESC_SZ + (sizeof(mgmt_frame->u.auto_rate)+ 2 * mgmt_frame->u.auto_rate.num_supported_rates)));
...
ONEBOX_STATUS send_bgscan_probe_req( PONEBOX_ADAPTER adapter, struct ieee80211_node *ni, uint16 flags)
{
	onebox_mac_frame_t *mgmt_frame;
	uint8  pkt_buffer[MAX_MGMT_PKT_SIZE];
	struct ieee80211com *ic = &adapter->vap_com;
...
void send_bgscan_host_triggered_bgscan(struct ieee80211vap *vap)
{
		...
		status = send_bgscan_probe_req(adapter, vap->iv_bss, (vap->hal_priv_vap->bgscan_params_ioctl.bg_cmd_flags|BIT(4)));
}
