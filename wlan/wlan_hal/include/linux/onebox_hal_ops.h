...
struct onebox_net80211_operations 
{
	...
	void (*onebox_send_scan_done)(struct ieee80211vap *);
	...
	int 	 (*onebox_cfg80211_attach) (struct net_device *vap_dev,
										void   *dev_ptr,
			                            int size);
#endif
...
{
	uint32 (*onebox_core_init)(PONEBOX_ADAPTER adapter);
	uint32 (*onebox_core_deinit)(PONEBOX_ADAPTER adapter);
	int (*onebox_core_xmit)(PONEBOX_ADAPTER adapter,netbuf_ctrl_block_t  *netbuf_cb);
	uint32 (*onebox_update_tx_status)(PONEBOX_ADAPTER adapter,struct tx_stat_s *tx_stat);
	uint32 (*onebox_tx_data_done)(PONEBOX_ADAPTER adapter,netbuf_ctrl_block_t *netbuf_cb);
	uint32 (*onebox_indicate_pkt_to_net80211)(PONEBOX_ADAPTER adapter,netbuf_ctrl_block_t *netbuf_cb, int8 rssi, int8 chno);
	uint32 (*onebox_indicate_pkt_to_core)(PONEBOX_ADAPTER adapter,netbuf_ctrl_block_t *netbuf_cb);
#ifdef BYPASS_RX_DATA_PATH
	uint8 (*onebox_bypass_rx_data_pkt)(PONEBOX_ADAPTER adapter,netbuf_ctrl_block_t *netbuf_cb);
#endif
	void (*onebox_core_qos_processor)(PONEBOX_ADAPTER adapter);
        void (*onebox_vap_load_beacon)(PONEBOX_ADAPTER adapter,int32 vap_id);
	...
	void (*onebox_core_radiotap_tx)(PONEBOX_ADAPTER adapter, struct ieee80211vap *vap, netbuf_ctrl_block_t *netbuf_cb);
	void (*onebox_dump)(int32 dbg_zone_l, PUCHAR msg_to_print_p,int32 len);
	void (*onebox_mic_failure)(PONEBOX_ADAPTER adapter,uint8 *msg);
	ONEBOX_STATUS (*onebox_stats_frame)(PONEBOX_ADAPTER adapter);
	PWR_STATUS (*onebo_load_deep_sleep) (PONEBOX_ADAPTER adapter, uint16 cmd);
	int (*onebox_start_per_tx)(PONEBOX_ADAPTER adapter);
	ONEBOX_STATUS (*onebox_start_per_burst)(PONEBOX_ADAPTER adapter);
	int  (*onebox_core_bt_xmit) (PONEBOX_ADAPTER adapter, netbuf_ctrl_block_t *netbuf_cb);
  void (*onebox_net80211_attach)(PONEBOX_ADAPTER adapter);
...
struct onebox_devdep_operations
{
	...
	ONEBOX_STATUS (*onebox_load_ta_instructions)(PONEBOX_ADAPTER adapter);
	ONEBOX_STATUS (*onebox_snd_cmd_pkt)(PONEBOX_ADAPTER  adapter, netbuf_ctrl_block_t *netbuf_cb);
	//ONEBOX_STATUS (*onebox_snd_mgmt_pkt)(PONEBOX_ADAPTER adapter, 
   //                                          netbuf_ctrl_block_t *netbuf_cb);
	ONEBOX_STATUS (*onebox_send_beacon)(PONEBOX_ADAPTER adapter, 
                                            netbuf_ctrl_block_t *netbuf_cb,
	                                    struct core_vap *core_vp,
                                            int8 dtim_beacon);
	ONEBOX_STATUS (*onebox_send_data_pkt)(PONEBOX_ADAPTER adapter,
                                              netbuf_ctrl_block_t *netbuf_cb,
                                              int8 q_num);
	ONEBOX_STATUS (*onebox_send_pkt)(PONEBOX_ADAPTER adapter,
                                   netbuf_ctrl_block_t *netbuf_cb);
	ONEBOX_STATUS (*onebox_device_init)(PONEBOX_ADAPTER adapter);
	ONEBOX_STATUS (*onebox_device_deinit)(PONEBOX_ADAPTER adapter);
	ONEBOX_STATUS (*onebox_set_channel)(PONEBOX_ADAPTER adapter, uint16 ch_no);
	ONEBOX_STATUS (*onebox_mgmt_send_bb_reset_req)(PONEBOX_ADAPTER adapter);
	int (*onebox_hal_set_sec_wpa_key)(PONEBOX_ADAPTER adapter,const struct ieee80211_node *ni_sta, uint8 key_type);
	uint32 (*onebox_hal_send_sta_notify_frame)(PONEBOX_ADAPTER adapter, struct ieee80211_node *sta_addr, int32 sta_id);
	void (*onebox_schedule_pkt_for_tx)(PONEBOX_ADAPTER adapter);
	void (*onebox_sdio_scheduler_thread)(void *data);
//	int (*onebox_send_qos_conf_frame)(PONEBOX_ADAPTER adapter,struct chanAccParams wme);
	int (*onebox_program_bb_rf) (PONEBOX_ADAPTER adapter);
	ONEBOX_STATUS (*onebox_update_device_op_params) (PONEBOX_ADAPTER adapter);
//	ONEBOX_STATUS (*onebox_start_tx_rx) (PONEBOX_ADAPTER adapter);
	ONEBOX_STATUS (*onebox_start_autorate_stats) (PONEBOX_ADAPTER adapter);
	ONEBOX_STATUS (*onebox_hal_load_key)(PONEBOX_ADAPTER adapter, uint8 *data, uint16 key_len, 
	                                     uint16 offset, uint8 key_type, uint8 key_id, uint32 cipher, struct ieee80211vap *vap);
//	ONEBOX_STATUS (*onebox_set_vap_capabilities)(PONEBOX_ADAPTER adapter, int unit, 
//	                    enum ieee80211_opmode opmode, uint8 *bssid);
	ONEBOX_STATUS (*onebox_set_vap_capabilities)(struct ieee80211vap *vap, uint8 status);
	//uint16* (*onebox_hw_queue_status)(PONEBOX_ADAPTER adapter);
	ONEBOX_STATUS (*onebox_band_check)(PONEBOX_ADAPTER adapter);
	ONEBOX_STATUS (*onebox_set_bb_rf_values)(PONEBOX_ADAPTER adapter, struct iwreq *wrq);
	ONEBOX_STATUS (*onebox_cw_mode)(PONEBOX_ADAPTER adapter, uint8 mode);
	ONEBOX_STATUS (*onebox_send_internal_mgmt_frame)(PONEBOX_ADAPTER adapter, uint16 *addr, uint16 len);
	ONEBOX_STATUS (*onebox_eeprom_rd)(PONEBOX_ADAPTER adapter);
	ONEBOX_STATUS (*onebox_flash_write)(PONEBOX_ADAPTER adapter);
...
#endif
	ONEBOX_STATUS (*onebox_program_ant_sel)(PONEBOX_ADAPTER adapter, uint8 value);
