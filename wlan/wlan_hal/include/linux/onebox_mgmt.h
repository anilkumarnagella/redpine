...
#define STATION_STATISTICS_RESP      0x0F9E //FIXME
...
typedef struct onebox_mac_frame_s 
{
	...
	union 
	{
		...
		struct 
		{
			uint16  reg_addr;
			uint16  bb_prog_vals;
		} bb_prog_req[100]; //FIXME: optimise this as some sets may not have the max vals

		struct 
		{
			uint16 rf_prog_vals[100]; //FIXME: optimise this as some sets may have less vals
		} rf_prog_req;

		struct 
		{
			uint16  bb_buf_vals[100];
		} bb_buf_req; //FIXME: optimise this as some sets may not have the max vals
...
int32 onebox_send_reset_mac(PONEBOX_ADAPTER adapter);
int32 onebox_send_bt_mode_config(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_send_bt_state_config(PONEBOX_ADAPTER adapter, 
				    		uint16 esco_pl_reg,
				    		uint32 LAP,
				    		uint32 UNAP,
				    		uint8 dev_state,
				    		uint32 clk_offset);
ONEBOX_STATUS onebox_send_bt_state_config_per(PONEBOX_ADAPTER adapter); 
ONEBOX_STATUS onebox_send_bt_state_config_le(PONEBOX_ADAPTER adapter,
                                                uint8 dev_state); 

ONEBOX_STATUS onebox_send_fhs_pkt(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_send_poll_pkt(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_send_null_pkt(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_send_dh1_pkt(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_process_dm1_pkt(PONEBOX_ADAPTER adapter, uint8 *mpkt);
...
ONEBOX_STATUS onebox_send_lmp_feature_res(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_send_lmp_feature_req(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_send_lmp_name_req(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_send_lmp_name_res(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_send_lmp_host_conn_req(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_send_bt_le_connect_req(PONEBOX_ADAPTER adapter);


VOID onebox_bt_pkt_rcvd(PONEBOX_ADAPTER adapter,uint8  *pkt,uint16 pktLen);
VOID onebox_le_bt_pkt_rcvd(PONEBOX_ADAPTER adapter,uint8  *pkt,uint16 pktLen);
ONEBOX_STATUS onebox_send_le_ll_data(PONEBOX_ADAPTER adapter);

/* LE pkt template loading */
ONEBOX_STATUS onebox_send_bt_load_pkt_templates(PONEBOX_ADAPTER adapter, uint8 pkt_type); 
ONEBOX_STATUS onebox_load_le_adv_ind(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_load_le_scan_req(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_load_le_scan_rsp(PONEBOX_ADAPTER adapter);
ONEBOX_STATUS onebox_load_le_connect_req(PONEBOX_ADAPTER adapter;
#endif
...
ONEBOX_STATUS flash_write(PONEBOX_ADAPTER adaprer);
...
int send_qos_conf_frame(PONEBOX_ADAPTER adapter, struct chanAccParams wme);
...
//ONEBOX_STATUS set_vap_capabilities(PONEBOX_ADAPTER adapter, int unit, 
...
ONEBOX_STATUS send_bgscan_probe_req( PONEBOX_ADAPTER adapter, struct ieee80211_node *ni, uint16 cmd_flags);
