...
ONEBOX_STATUS onebox_send_rx_filter_frame(PONEBOX_ADAPTER, uint16_t rx_filter_word);
...
ONEBOX_STATUS onebox_send_auto_rate_request (PONEBOX_ADAPTER, struct ieee80211_node *);

/*************** Security related prototypes ********************/

int onebox_keyset(PONEBOX_ADAPTER adapter, 
                  const struct ieee80211_key *k,
                  const uint8 mac0[IEEE80211_ADDR_LEN], 
                  struct ieee80211_node *bss);
uint16 key_alloc_2pair(PONEBOX_ADAPTER adapter);
uint16 key_alloc_pair(PONEBOX_ADAPTER adapter);
uint16 key_alloc_single(PONEBOX_ADAPTER adapter);
...
int onebox_cfg80211_attach(struct net_device *vap_dev, void *dev_ptr, int size);
#endif
