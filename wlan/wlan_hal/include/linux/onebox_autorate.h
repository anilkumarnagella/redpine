...
{
	int ratectl_id;
	struct rsi_ratectrl *(*attach)(PONEBOX_ADAPTER *sc);
	void (*detach)(PONEBOX_ADAPTER *arc);
	/* Register proc entries with a VAP */
	void (*dynamic_proc_register)(struct ieee80211vap *vap);

	void (* update_ratestats)(PONEBOX_ADAPTER adapter);
	void (*findrate)(struct onebox_priv *sc, 
	 ...
	 * in ibss mode) */
	void (* newassoc)(PONEBOX_ADAPTER sc, struct ieee80211_node *rn, int isnew);
