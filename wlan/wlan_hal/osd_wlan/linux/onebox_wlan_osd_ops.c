                     ...
                     void *context)
{
	...
	atomic_set(&adapter->txThreadDone,0);
...
int kill_wlan_thread(PONEBOX_ADAPTER adapter)
{
	...
	printk("IN KILL WLAN THREAD %s %d\n", __func__, __LINE__);
	...
	atomic_inc(&adapter->txThreadDone);
	ONEBOX_DEBUG(ONEBOX_ZONE_INFO,(TEXT("In %s Before setting event\n"),__func__));
	os_intf_ops->onebox_set_event(&(adapter->sdio_scheduler_event));
...
int kill_per_thread(PONEBOX_ADAPTER adapter)
{
	...
	if(adapter->total_per_pkt_sent)
	{
		handle = &adapter->sdio_scheduler_thread_handle_per;
		handle->kill_thread = 1;
		wait_for_completion(&adapter->txThreadComplete);
		handle->thread_id = 0;
	}
...
{
	PONEBOX_ADAPTER adapter = (PONEBOX_ADAPTER)pContext;
	ONEBOX_STATUS status;
	int  no_of_fragments= 0; 
	no_of_fragments = adapter->no_of_per_fragments = adapter->endpoint_params.aggr_count;
	do  
	{  
		status = adapter->core_ops->onebox_start_per_burst(adapter);
                no_of_fragments--;
		//	} while (!handle->kill_thread && !status ); 
	} while (no_of_fragments && adapter->endpoint_params.aggr_enable); 
