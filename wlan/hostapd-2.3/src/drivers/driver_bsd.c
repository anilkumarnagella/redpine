	...
	else /* The above event also should be done in the following way. But this is temporary patch hence left */
	{
		...
		switch(custom_cmd_type)
		{
			case DRIVER_EVENT_ASSOC:
			{
				...
				if(drv->is_ap)
				{
					printf("Going to AP mode cases\n");
					ies_len = *(uint16_t *)(custom + 10);
					ies = (uint8_t *)(custom + 12);
				}
				else
				{
					printf("Going to else mode cases\n");
					ies_len = 0;
					ies = NULL;
				}
					    ...
					    char *data, int len)
{
	...
	printf("Received event in hostapd\n");
	{
		int i;
		for(i = 0; i < len ; i++)
		{
...
bsd_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
	...
	//drv->route = socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE);
	if (drv->route < 0) {
