	...
	switch (pkt_type)
	{
		case HCI_COMMAND_PKT:
		{
			...
			switch(ogf)
			{
				...
				{
					ONEBOX_DEBUG(ONEBOX_ZONE_ERROR, (TEXT("VENDOR SPECIFIC COMMAND\n")));
					break;
				}
				default:
				{
...
void print_hci_cmd_type(PONEBOX_ADAPTER adapter, netbuf_ctrl_block_t *netbuf_cb)
{
  ...
  pkt_type = netbuf_cb->bt_pkt_type;
	switch (pkt_type)
	{
      ...
      adapter->osi_bt_ops->onebox_dump(ONEBOX_ZONE_ERROR, netbuf_cb->data, netbuf_cb->len);
			ONEBOX_DEBUG(ONEBOX_ZONE_ERROR, (TEXT("\n")));
			break;
		}
		case HCI_ACLDATA_PKT:
...
int32 send_bt_pkt(PONEBOX_ADAPTER adapter, 
		   netbuf_ctrl_block_t *netbuf_cb)
{
	...
	if(adapter->os_intf_ops->onebox_change_hdr_size(netbuf_cb, FRAME_DESC_SZ) == NULL)
	{
	...
	netbuf_cb->tx_pkt_type = BT_TX_Q;
	{
		...
		ONEBOX_DEBUG(ONEBOX_ZONE_ERROR, (TEXT("TX PACKET LENGTH = %d\n"), netbuf_cb->len));
		//print_rx_hci_cmd_type(adapter, *(uint16 *)&netbuf_cb->data[FRAME_DESC_SZ], netbuf_cb->bt_pkt_type);
		print_hci_cmd_type(adapter, netbuf_cb);
		ONEBOX_DEBUG(ONEBOX_ZONE_ERROR, (TEXT("\nTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTXTX\n")));
	}
	/* Sending packet to the hal interface layer */
	status = adapter->onebox_send_pkt_to_coex(netbuf_cb, BT_Q);
