...
int main (int argc, char *argv[])
{
  ...
  int country;
	...
	switch (cmdNo)
	{
		...
		break;
		case RSI_GET_AUTORATE_STATS:
		{
			strcpy (ifName, argv[1]);
			memset (&wrq, 0, sizeof (struct iwreq));
			strncpy (wrq.ifr_name, ifName, IFNAMSIZ);
			memset (macaddr, 0, 6);
			/* To convert the mac address into hexadecimal */
			byteconversion (argv[3], macaddr);
			wrq.u.data.pointer = malloc (sizeof (struct minstrel_node) +
			                     sizeof (struct minstrel_rate) * MAX_RATES);
			wrq.u.data.length = sizeof (struct minstrel_node);
			memcpy (wrq.u.data.pointer, macaddr, IEEE80211_ADDR_LEN);
			if (ioctl (sfd, RSIIOCGETSTATS, &wrq) < 0)
			{
				ONEBOX_PRINT ("Error while getting autorate stats\n");
				return -1;
			}
			ar_stats = (struct minstrel_node *) wrq.u.data.pointer;
			ONEBOX_PRINT ("num rates= %d\n", ar_stats->n_rates);

			ONEBOX_PRINT("max_tp_rate= %d, max_tp_rate2= %d, max_prob_rate=%d\n\n",
			             ar_stats->max_tp_rate, ar_stats->max_tp_rate2,
			             ar_stats->max_prob_rate);
			for (ii = 0; ii < ar_stats->n_rates; ii++)
			{
				mr =(wrq.u.data.pointer + sizeof (struct minstrel_node) +
				     ii * sizeof (struct minstrel_rate));
				ONEBOX_PRINT("rate*2 = %03d cur_prob= %08d tot_prob=%08d cur_tp=%03d Attempts =%08d success= %08d last_attempts = %04d last_succ= %04d perf_time= %010d\n",
				              mr->bitrate, mr->cur_prob, mr->probability,
				              (mr->cur_tp / 1000000), mr->att_hist, mr->succ_hist,
				              mr->last_attempts, mr->last_success, mr->perfect_tx_time);
			}
		}
		break;
		case VAPSTATS:
		...
		case RSI_SET_BGSCAN:
				...
				{
					 if(((atoi(argv[i]) >= 1) && (atoi(argv[i]) <= 13)) || 
			...
			if (!cw_mode)
			{
				switch (cw_type)
				{
					...
					case 5:
						...
						{
							ONEBOX_PRINT("Error while setting beacon interval value %s\n",ifName);
						}
						break;	
					case 9:
...
getcmdnumber (char *command)
{
	...
	if (!strcmp (command, "get_autorate_stats"))
	{
		return RSI_GET_AUTORATE_STATS;
	}
...
get_driver_state (char *ptr)
{
	...
	fs = system ("test -f /proc/onebox-mobile/stats");
	//ONEBOX_PRINT("the get driver function..\n");
	if (!fs)
	{
		fp = popen
		     ("cat /proc/onebox-mobile/stats | grep -i FSM_ | cut -f2 -d ' ' | cut -f1 -d '('",
		      "r");
	...
	{
		ONEBOX_PRINT ("The required file /proc/onebox-mobile/stats is not Found\n");
		str_out[0] = '\0';
	}
...
void usage ()
{
	...
	ONEBOX_PRINT
	        ("Usage: onebox_util base_interface get_autorate_stats macaddr\n");
	ONEBOX_PRINT
