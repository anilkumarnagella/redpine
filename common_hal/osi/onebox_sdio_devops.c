	...
	uint32 block_size, uint8 *ta_firmware)
{
	...
	if (instructions_sz % block_size)
	{
		...
		adapter->coex_osi_ops->onebox_dump(ONEBOX_ZONE_ERROR, (uint8 *)temp_buf, instructions_sz % block_size);
	}
			...
			switch (interrupt_type)
			{
				case BUFF_STATUS_UPDATE:
					...
					if (d_assets.techs[WLAN_ID].drv_state == MODULE_ACTIVE) {
						if (d_assets.techs[WLAN_ID].onebox_get_buf_status) {
							d_assets.techs[WLAN_ID].onebox_get_buf_status(device_buf_status);
						}
					}
				break;
				...
				case FIRMWARE_ASSERT_IND:
					...
					{  
						...
						if (d_assets.techs[WLAN_ID].drv_state == MODULE_ACTIVE) 
								d_assets.techs[WLAN_ID].wlan_dump_mgmt_pending();
