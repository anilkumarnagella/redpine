...
METADATA metadata_flash_content[] = {
                                 ...
                                 {"RS9113_WLAN_BT_CLASSIC.rps", 0x00010000}, 
                                 {"RS9113_WLAN_ZIGBEE.rps", 0x00010000},
                                 {"RS9113_WLAN_BT_LE.rps", 0x00010000}, 
                                 {"RS9113_WLAN_SNIFFER_QSPI.rps", 0x00010000} 
};
...
METADATA metadata_per_flash_content[] = {
                                 {"flash_content_dummy", 0x00010000}, 
                                 {"RS9113_WLAN_PER_QSPI.rps", 0x00010000}, 
                                 {"RS9113_WLAN_BT_PER_CLASSIC.rps", 0x00010000}, 
                                 {"RS9113_WLAN_ZIGBEE_PER.rps", 0x00010000},
                                 {"RS9113_WLAN_BT_LE_PER.rps", 0x00010000} 
};
...
static ONEBOX_STATUS load_fw(PONEBOX_ADAPTER adapter, METADATA metadata_rcv )
{
	...
	if(ONEBOX_STATUS_FAILURE == load_data_master_write(adapter, base_address, instructions_sz, block_size, ta_firmware)) {
...
int  bl_cmd(PONEBOX_ADAPTER adapter, uint8 cmd, uint8 exp_resp, char *str)
{
	...
	if(bl_write_cmd(adapter, cmd, exp_resp, &regout_val) < 0) {
		printk("%s:%d %s Command %0x writing failed..\n",__func__,
				__LINE__, str, cmd);
		goto fail;
	}
...
int bl_write_cmd(PONEBOX_ADAPTER adapter, uint8 cmd, uint8 exp_resp, uint16 *cmd_resp)
{
	...
	if(adapter->bl_timer_expired) {
		printk("%s:%d Command %0x REGIN reading timed out..\n",__func__,
				__LINE__,cmd);
		goto fail;
	}

	printk("Issuing write into Regin regin_val:%0x sending cmd :%0x\n",regin_val, (cmd | REGIN_INPUT << 8));
	...
	printk("waiting for regout to become valid\n");
	while(!adapter->bl_timer_expired) {
		regout_val = 0;
		if(adapter->osd_host_intf_ops->onebox_master_reg_read(adapter, 
					SWBL_REGOUT, &regout_val, 2) < 0) {
			printk("%s:%d Command %0x REGOUT reading failed..\n",__func__,
					__LINE__,cmd);
			goto fail;
		}
	...
	if(adapter->bl_timer_expired) {
		printk("%s:%d Command %0x REGOUT reading timed out..\n",__func__,
				__LINE__,cmd);
		goto fail;
	}
	...
	*cmd_resp = regout_val;
	...
	printk("Invalidating regout\n");
	if((adapter->osd_host_intf_ops->onebox_master_reg_write(adapter, 
					SWBL_REGOUT, (cmd | REGOUT_INVALID << 8), 2)) < 0) {
		printk("%s:%d Command %0x REGOUT writing failed..\n",__func__,
				__LINE__,cmd);
		goto fail;
	}

	if(output == exp_resp) {
		printk("%s:%d Recvd Expected resp %x for cmd %0x\n",__func__,__LINE__, output, cmd);
	} else {
		printk("%s:%d Recvd Unexpected resp %x for cmd %0x\n",__func__,__LINE__, output, cmd);
		goto fail;
	}
	return ONEBOX_STATUS_SUCCESS;
...
#endif
	{
		printk("%s:%d Failed to load Version/CRC structure\n",__func__,__LINE__);
		goto fail;
	}
	return ONEBOX_STATUS_SUCCESS;
...
ONEBOX_STATUS load_ta_instructions(PONEBOX_ADAPTER adapter)
{
	...
	printk("**DRIVER MODE is %d COEX MODE is %d and loading file %s**\n", adapter->Driver_Mode, 
			adapter->coex_mode,
			metadata_p->name);
	status = load_fw(adapter, *metadata_p);
	if(status == ONEBOX_STATUS_FAILURE) {
		printk("%s:%d Unable to load pmemdata...\n",__func__,__LINE__);
		goto fail;
	}

	if(adapter->fw_load_mode == FLASH_RAM_NO_SBL) {
		status = ta_reset_ops(adapter);
		if(status == ONEBOX_STATUS_FAILURE) {
			printk("%s:%d Unable to put ta in reset\n",__func__,__LINE__);
			goto fail;
		}
	}
	return ONEBOX_STATUS_SUCCESS;
...
ONEBOX_STATUS load_fw_thru_sbl(PONEBOX_ADAPTER adapter)
{
	...
	if(adapter->Driver_Mode == QSPI_FLASHING &&
		...
		printk("Invalid fw load mode configured %d\n",adapter->fw_load_mode);
		goto fail;
	}
...
RPS_IMAGE_HEADER:
	...
	adapter->coex_osi_ops->onebox_dump(ONEBOX_ZONE_ERROR, &flash_content[0], 16);
	status = write_bl_header(adapter, flash_content, content_size);
	if(status == ONEBOX_STATUS_SUCCESS) {
		printk("RPS Image Header Written Successfully\n");;
		goto CHECK_CRC_CMD;
	} else {
		printk("RPS Image header loading failed\n");
		goto fail;
	}
...
CHECK_CRC_CMD:
	...
	if(bl_write_cmd(adapter, CHECK_CRC, CMD_PASS, &regout_val) < 0) {
		bl_cmd_stop_timer(adapter);
		printk("%s:%d CHECK_CRC Command writing failed..\n",__func__,__LINE__);
		if((regout_val & 0xff) == CMD_FAIL) {
			printk("CRC Fail.. Proceeding to Upgrade mode\n");
			goto FW_UPGRADE;
		}
	}
	bl_cmd_stop_timer(adapter);
	if(adapter->fw_load_mode == FLASH_RAM_SBL ||
		...
		printk("CRC Pass..Issuing Polling Command\n");
		goto POLLING_MODE_CMD;
	} else {
		printk("Entered wrong path in fw_load_mode :%d\n",adapter->fw_load_mode);
		goto fail;
	}
...
LOAD_IMAGE_CMD:
	...
	printk("Load Image command passed..\n");
	goto success;
...
POLLING_MODE_CMD:
	...
	if(adapter->fw_load_mode == FULL_FLASH_SBL) {
		printk("Polling mode command passed.. Issuing Load hosted fw cmd\n");
		goto LOAD_IMAGE_CMD;
	} else {    
		printk("Polling mode command passed.. Loading pmemdata\n");
		goto LOAD_PMEMDATA;
	}
...
LOAD_PMEMDATA:
	...
	if(status == ONEBOX_STATUS_FAILURE) {
		printk("%s:%d PMEMDATA Loading Failed\n",__func__,__LINE__);
		goto fail;
	} else {
		printk("pmemdata loading successful.. Issuing Jump to Zero command\n");
		goto JUMP_TO_ZERO;
	}
...
JUMP_TO_ZERO:
	...
	printk("Jump to zero command successful\n");
	goto success;
...
FW_UPGRADE:
	...
	printk("Burn Command Pass.. Upgrading the firmware\n");
	status = adapter->coex_osi_ops->onebox_auto_fw_upgrade(adapter, flash_content, content_size);
	if(status == ONEBOX_STATUS_SUCCESS) {
		goto LOAD_IMAGE_CMD;
	} else {
		printk("****FIRMWARE UPGRADATION FAILED****\n");
		goto AUTO_READ_MODE_CMD;
	}

BURN_BOOTLOADER:
	printk("Reading calib data using Master Read\n");
	...
	if(read_flash_content(adapter, adapter->calib_data, 
		...
		printk("Failed to read calib data using master read\n");
		goto fail;
	}
