...
#ifdef USE_USB_INTF
#define FLASH_READ_CHUNK_SIZE  ((2 * 1024) - 4)
#else
#define FLASH_READ_CHUNK_SIZE  (2 * 1024)
#endif
...
uint32 read_flash_capacity(PONEBOX_ADAPTER adapter)
{
	...
	if(adapter->osd_host_intf_ops->onebox_master_reg_read(adapter, 
		...
		printk("%s:%d Flash size reading failed..\n",__func__,
				__LINE__);
		return 0;
	}
	printk("Flash capacity is %d KiloBytes\n",flash_sz);
...
ONEBOX_STATUS manual_flash_write(PONEBOX_ADAPTER adapter)
{
	...
	adapter->os_intf_ops->onebox_memcpy(&spi_config, &spi_default_configs, sizeof(spi_config_t));
	{
		...
		mgmt_frame->desc_word[2] |= ONEBOX_CPU_TO_LE16(BIT(10));
#endif
		printk("B4 Final Writing %p\n",mgmt_frame);
		printk("Flash offset 0x%x, %d\n",adapter->flash_offset, adapter->flash_offset);
	...
	printk(" temp Writing %p\n",temp);
	adapter->os_intf_ops->onebox_memcpy(temp, &spi_config, sizeof(spi_config_t));
	adapter->coex_osi_ops->onebox_dump(ONEBOX_ZONE_ERROR, (PUCHAR)&spi_default_configs, sizeof(spi_config_t));
	
	printk(" Final Writing %p\n",mgmt_frame);
	mgmt_frame->desc_word[0] |= ONEBOX_CPU_TO_LE16(ONEBOX_WIFI_MGMT_Q << 12 | (extended_desc*3));
	adapter->coex_osi_ops->onebox_dump(ONEBOX_ZONE_ERROR, (PUCHAR)mgmt_frame, 
...
#define DELAYS_LEN 12
	...
	{
	  	printk("Flash erase sent");
		mgmt_frame->desc_word[2] |= ONEBOX_CPU_TO_LE16(BIT(10));
	}  
	if(adapter->eeprom_init)
	{
	  	printk("spi init sent");
...
ONEBOX_STATUS flash_read(PONEBOX_ADAPTER adapter)
{
	...
	adapter->coex_osi_ops->onebox_dump(ONEBOX_ZONE_ERROR, (PUCHAR)mgmt_frame, pkt_len);
	status = adapter->osi_host_intf_ops->onebox_host_intf_write_pkt(adapter, 
			(uint8 *)mgmt_frame, pkt_len,
			WLAN_TX_M_Q);
	adapter->os_intf_ops->onebox_mem_free(pkt_buffer);
				       ...
				       )
{
	...
	{
			ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT ("%s: BLOCK/SECTOR WRITING SUCCESSFULL \n"), __FUNCTION__));
				     ...
				     )
{
	...
	printk("Number of loops required is %d\n",num_loops);
	for (idx = 0; idx < num_loops; idx++)
	{
		if(instructions_sz < flash_chunk_size) {
			chunk_size = instructions_sz;
		...
		if ((idx % (FLASH_SECTOR_SIZE/FLASH_WRITE_CHUNK_SIZE)) == 0) {
			printk("idx is %d and hence calling erase\n", idx);
			adapter->eeprom_erase = 1;
		}
 		else { 
		...
		printk("idx is %d and chunk size is %d\n", idx, chunk_size);
		adapter->eeprom.offset = eeprom_offset;
		adapter->eeprom.length = chunk_size;
		...
		if (status == ONEBOX_STATUS_SUCCESS)
		{
		  ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT ("%s: BLOCK/SECTOR WRITING SUCCESSFULL \n"), __FUNCTION__));
		  adapter->os_intf_ops->onebox_reset_event(&adapter->flash_event);
		  printk("idx + 1 is %d and eeprom_cfm_count is %d\n",idx + 1, adapter->eeprom_cfm_count);
	...
	printk("idx is %d and eeprom_cfm_count is %d\n",idx, adapter->eeprom_cfm_count);
	return ONEBOX_STATUS_SUCCESS; 
}
		...
		)
{
	...
	uint32 flash_chunk_size = FLASH_READ_CHUNK_SIZE;
	...
	for (idx = 0; idx < num_loops; idx++)
	{
		...
		if(read_mode == EEPROM_READ_MODE) {
			adapter->eeprom.offset = eeprom_offset;
			printk("eeprom offset is %x\n", eeprom_offset);
			adapter->eeprom.length = chunk_size;
			status = adapter->coex_osi_ops->onebox_flash_read(adapter);
			if (status == ONEBOX_STATUS_SUCCESS) {
				ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT ("%s: BLOCK/SECTOR READING SUCCESSFULL \n"), __FUNCTION__));
				adapter->os_intf_ops->onebox_reset_event(&adapter->flash_event);
				status = adapter->os_intf_ops->onebox_wait_event(&(adapter->flash_event),  12000); 
				if(!status) {
					ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,
...
ONEBOX_STATUS read_flash_content(PONEBOX_ADAPTER adapter, uint8 *temp_buf, uint32 address, uint32 len)
{
	...
	address &= 0xFFFF; 
	//FIXME: FOR SDIO MODE Please look into this.
...
static ONEBOX_STATUS read_calib_data(PONEBOX_ADAPTER adapter, uint8 *flash_content, uint8 read_mode)
{
	...
	if(temp_buffer[MAGIC_WORD_OFFSET_1] != MAGIC_WORD ||
		...
		printk("Invalid Calibration data present.. Skipping Flashing\n");
		goto fail;
	}
	...
	if(temp_buffer[EEPROM_VERSION_OFFSET] >= EEPROM_VER6) {
					...
					if(calc_crc != read_crc) {
									printk("CRC verification Failed.. calc_crc:%x and read_crc:%x"
																	"Skipping Flashing\n",calc_crc,read_crc );
									goto fail;
					}
	}
	...
	if(adapter->flashing_mode == SWBL_FLASHING_NOSBL) {
		printk("Started writing calibration data to a file\n");
		...
		if(status == ONEBOX_STATUS_FAILURE) {
			printk("Failed to write calib data to a file\n");
		}
	}
	...
	ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT ("%s: Successfully overwritten Calib data\n"), __FUNCTION__));
...
ONEBOX_STATUS manual_load_flash_content(PONEBOX_ADAPTER adapter, METADATA metadata_rcv)
{
	...
	if(instructions_sz > MAX_FLASH_FILE_SIZE) {
		printk("Flash Content size is more than 400K i.e %u and so, not flashing\n",MAX_FLASH_FILE_SIZE);
	...
	if (adapter->flashing_mode == QSPI_UPDATE) {
		printk("First 4K in new Flash File \n ");
		...
		read_flash_content(adapter,
				...
				CALIB_DATA_SIZE);
		printk("First 4K from existing Flash on card \n");
		adapter->coex_osi_ops->onebox_dump(ONEBOX_ZONE_ERROR, ta_firmware, EEPROM_DATA_SIZE);
	...
	if (adapter->flashing_mode == QSPI_FLASHING) {
		...
		printk("flash start address is %08x\n",flash_start_address);

		if(flash_start_address <= FLASH_SECTOR_SIZE) {
			printk("Flash Start Address is less than 4K.. So not flashing\n");
			adapter->os_intf_ops->onebox_vmem_free(ta_firmware_temp);
			return ONEBOX_STATUS_FAILURE;
		}

		if(flash_start_address % FLASH_SECTOR_SIZE) {
			printk("Flash Start Address is not multiple of 4K.. So not flashing\n");
			adapter->os_intf_ops->onebox_vmem_free(ta_firmware_temp);
			return ONEBOX_STATUS_FAILURE;
		}
		...
		if(flash_capacity <= 0) {
			printk("Unable to read flash size from EEPROM\n");
			adapter->os_intf_ops->onebox_vmem_free(ta_firmware_temp);
			return ONEBOX_STATUS_FAILURE;
		}

		if(flash_start_address + instructions_sz > flash_capacity) {
			printk("Flash Content will cross max flash size.. So not flashing\n");
			adapter->os_intf_ops->onebox_vmem_free(ta_firmware_temp);
			return ONEBOX_STATUS_FAILURE;
		}
#endif
	...
	ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("instructions_sz:%d\n"),
				instructions_sz));
	ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("num_flash: %d\n"),num_flash));

	for (kk = 0; kk <= num_flash; kk++,flash_indx += FLASH_BLOCK_SIZE)
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("flash_indx: %d\n"),flash_indx));
		if (adapter->qspi_flashing)
		{
			if (kk != num_flash)
			{	
				instructions_sz = FLASH_BLOCK_SIZE;
				ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("QSPI instructions_sz:%d\n"),
							instructions_sz));
			}
			else
			{
		...
		if (!instructions_sz)
			continue;
		ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("QSPI metadata_rcv.address :0x%x\n"),
					metadata_rcv.address));
		base_address = metadata_rcv.address;

		if(ONEBOX_STATUS_FAILURE == load_data_master_write(adapter, 
								   base_address, 
								   instructions_sz, 
								   block_size, 
								   ta_firmware)) {
	...
	printk("%s:Starting Flash Verification Process\n",__func__);
	...
	if(status == ONEBOX_STATUS_FAILURE) {
		printk("flash index = %d\n",flash_indx);
		printk("%s:FLASHING Firmware failed in FLASH VERIFICATION phase\n",__func__);
		adapter->os_intf_ops->onebox_vmem_free(ta_firmware_temp); 
		return ONEBOX_STATUS_FAILURE;
	}
	printk("%s:Flash Verification Process Completed Successfully\n",__func__);
	adapter->os_intf_ops->onebox_vmem_free(ta_firmware_temp);
	return ONEBOX_STATUS_SUCCESS;	
}
...
ONEBOX_STATUS auto_fw_upgrade(PONEBOX_ADAPTER adapter, uint8 *flash_content, uint32 content_size)
{
	...
	if(adapter->Driver_Mode == QSPI_FLASHING &&
		...
		if(flash_start_address < FW_IMAGE_MIN_ADDRESS) {
			printk("Fw image Flash Start Address is less than 68K.. So not flashing\n");
			goto fail;
		}
	}
	...
	ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("content_size:%d\n"),
				content_size));
	ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("num_flash: %d\n"),num_flash));
	...
	for (kk = 0; kk <= num_flash; kk++)
	{
		...
		{	
			...
			ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("QSPI content_size:%d\n"),
						content_size));
		}
		else
		{
			content_size = temp_content_size % FLASH_WRITE_CHUNK_SIZE;
			ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("QSPI LAST SECTOR BEING WRITTEN content_size:%d\n"),
						content_size));
			if (!content_size)
			{ 
				ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("INSTRUCTION SIZE ZEROOOOOOOOO: \n")));
	...
	printk("FW loading is done and Fw is running..\n");
	return ONEBOX_STATUS_SUCCESS;	
fail:
	return ONEBOX_STATUS_FAILURE;
}
...
ONEBOX_STATUS auto_load_flash_content(PONEBOX_ADAPTER adapter, METADATA metadata_rcv )
{
	...
	if(flash_start_address + instructions_sz > FLASH_MAX_ADDRESS) {
		printk("Flash Content will cross max flash size of 4MB.. So not flashing\n");
		goto fail;
	}
#endif
#if 0
	...
	if(status == ONEBOX_STATUS_FAILURE) {
		printk("FLASHING Firmware failed in FLASHING phase\n");
		goto fail;
	}
	...
	if(status == ONEBOX_STATUS_FAILURE) {
		printk("FLASHING Firmware failed in FLASH VERIFICATION phase\n");
		goto fail;
	}
