 ...
 */
ONEBOX_STATIC  int onebox_cmd52writebyte(struct mmc_card *card, unsigned int address, unsigned char byte)
{
 ...
 */
ONEBOX_STATIC int onebox_cmd52readbyte(struct mmc_card *card, unsigned int address, unsigned char *byte)
{
				struct mmc_command ioCmd;
				unsigned long   arg;
				int err;
 ...
 */
#define ADDR_ALIGN 0x03
#define is_aligned(a) (!((uint32)(a) & ADDR_ALIGN))
#define align_address(a) ((uint32)(a) & ~ADDR_ALIGN)

ONEBOX_STATUS master_reg_write(PONEBOX_ADAPTER adapter, uint32 addr, uint32 data, uint16 size)
{
				uint32 data1[2];
				uint32 *data_alligned;

				data_alligned = (uint32 *)align_address(&data1[1]);
				//	printk("%s %d addr=%x data=%x\n",__func__,__LINE__, addr, data );
...
ONEBOX_STATUS master_reg_read(PONEBOX_ADAPTER adapter, uint32 addr, uint32 *rd_buf, uint16 size)
{
				uint32 *data = NULL;
				uint16 ms_addr = 0;
				uint32 align[2] = {};
				...
				//printk(" In %s %d  data =%p &align[0]= %p &align[1]=%p\n", __func__, __LINE__, data, &align[0], &align[1]);
				if (1) {
								ms_addr = (addr >> 16);
								...
								addr = addr & 0xFFFF;
								printk(" In %s %d  setting msword =%x\n", __func__, __LINE__, ms_addr);

				}
				//	printk(" In %s %d  addr =%x\n", __func__, __LINE__,addr);
				...
				//	printk("In %s %d after doing read_multiple  *rd_data= %x, *data=%x\n", __func__, __LINE__, *rd_buf, *data);
				return ONEBOX_STATUS_SUCCESS;
}

void ulp_read_write(PONEBOX_ADAPTER adapter, uint16 addr, uint16 *data, uint16 len_in_bits)
{
				master_reg_write(adapter, GSPI_DATA_REG1, ((addr << 6) | (data[1] & 0x3f)), 2);
				master_reg_write(adapter, GSPI_DATA_REG0, *(uint16 *)&data[0], 2);
				onebox_gspi_init();
				master_reg_write(adapter, GSPI_CTRL_REG1, ((len_in_bits - 1) | GSPI_TRIG), 2);
				msleep(10);
}
...
#if 1
				printk("Writing disable to wakeup register in %s line %d\n", __func__, __LINE__);
...
#if KERNEL_VERSION_LESS_THAN_3_12_(28)
												host->ocr = resp;
#else
												card->ocr = resp;
#endif
...
int disconnect_assets(struct driver_assets *d_assets)
{
				...
				ONEBOX_DEBUG(ONEBOX_ZONE_ERROR, (TEXT("In %s %d  moving card state to GS_CARD_DETACH\n"), __func__, __LINE__));
				...
				if (d_assets->techs[WLAN_ID].drv_state == MODULE_ACTIVE) {
								if (d_assets->techs[WLAN_ID].deregister_flags) {
												...
												printk("In %s Line %d Waking up wait event for deregister\n", __func__, __LINE__);
...
#endif
{
				...
				os_intf_ops->onebox_mem_free(adapter->DataRcvPacket);
#if KERNEL_VERSION_BTWN_2_6_(18, 26)
				os_intf_ops->onebox_kill_thread(&adapter->sdio_scheduler_thread_handle); 
#elif KERNEL_VERSION_GREATER_THAN_2_6_(27)
				...
				ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT("%s: card reset done successfully\n"), __func__));
				/* Release host */
				onebox_sdio_release_host(pfunction);
...
int32 deregister_sdio_irq(PONEBOX_ADAPTER adapter)
{
				...
				onebox_sdio_claim_host(adapter->pfunction);
				s = onebox_sdio_release_irq(adapter->pfunction);
				if (s) {
								ONEBOX_DEBUG(ONEBOX_ZONE_INFO,
																(TEXT("%s: Unable to Deregister irq"), __func__));
				}	
				onebox_sdio_release_host(adapter->pfunction);
 ...
 * @return On success ONEBOX_STATUS_SUCCESS else ONEBOX_STATUS_FAILURE. 
 */
int write_register(PONEBOX_ADAPTER ext_adapter,uint8 reg_dmn,uint32 Addr,uint8 *data)
{
				struct driver_assets *d_assets = onebox_get_driver_asset();
				PONEBOX_ADAPTER adapter = (PONEBOX_ADAPTER)d_assets->global_priv;
#if KERNEL_VERSION_BTWN_2_6_(18, 22)
								...
								)
{
				...
				{
								printk("Stopping card writes\n");
								return ONEBOX_STATUS_FAILURE;
				}
				else if(adapter->stop_card_write == 1)
				{
								printk("Continue writes\n");
				...
				if (status != ONEBOX_STATUS_SUCCESS)
				{
								...
								adapter->coex_osi_ops->onebox_dump(ONEBOX_ZONE_ERROR, data, Count);
								adapter->stop_card_write = 2; /* To stop further card writes, as card might be removed. FIXME */
				}
				else
				{
...
void onebox_gspi_init(void)
{
				...
				master_reg_write(adapter, GSPI_CTRL_REG0, gspi_ctrl_reg0_val, 2);
}
