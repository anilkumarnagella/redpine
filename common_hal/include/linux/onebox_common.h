...
#define NAME_MAX_LENGTH         32
	...
	struct completion    coexThreadComplete;
	atomic_t   coexThreadDone;
...
#define MAX_RX_URBS 2
	...
	struct urb *tx_usb_urb[MAX_RX_URBS];
	struct usb_sg_request usb_sg;
	void*  usb_dev_sem;
	struct completion usb_rx_ThreadComplete;
	ONEBOX_EVENT usb_rx_event;
	ONEBOX_EVENT usb_tx_event;
	ONEBOX_EVENT usb_rx_scheduler_event;
	uint32          usb_rxThreadId;
	uint32          usb_rx_thread_exit;
	onebox_thread_handle_t    usb_rx_scheduler_thread_handle;
	struct completion    rxThreadComplete;
	atomic_t   rxThreadDone;
