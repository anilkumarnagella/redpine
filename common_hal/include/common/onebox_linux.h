...
/* FUNCTION PROTOTYPES */
int allocate_memory(PONEBOX_ADAPTER adapter, uint8 **vir_addr,uint32 memory_size, uint8 memory_type);
int free_memory(PONEBOX_ADAPTER adapter,
                uint8 *vir_addr,
                uint32 memory_size, 
                uint8 memory_type);
...
int init_rx_thread(onebox_thread_handle_t *handle, uint8 *name,
                ...
                void *context);
#endif
ONEBOX_STATUS onebox_start_per(PONEBOX_ADAPTER adapter);
void per_thread(void *pContext);
int kill_per_thread(PONEBOX_ADAPTER adapter);
...
int kill_wlan_thread(PONEBOX_ADAPTER adapter);
#ifdef USE_USB_INTF
int kill_rx_thread(PONEBOX_ADAPTER adapter);
#endif
#endif

int32 intr_request_irq (uint32 irq, int handler, ulong irqflags, const char *devname,
                        void *dev_id);
int get_pktq_len(PONEBOX_ADAPTER Adapter,int qnum);
void free_pkt(PONEBOX_ADAPTER Adapter,netbuf_ctrl_block_t *netbuf_cb,int status);
...
PUCHAR push_data(netbuf_ctrl_block_t *netbuf, uint16 len);
...
uint16 *extract_variable( PONEBOX_ADAPTER Adapter, uint8 *arr_name );
...
#endif
void initialize_timer(struct timer_list *timer, uint32 data, void *funtion, unsigned long timeout);
