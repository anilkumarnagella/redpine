...
int wait_queue_event(ONEBOX_EVENT *event, uint32 timeOut)
{
	...
	if (!timeOut)
	{
		wait_event_interruptible(event->eventQueue,
 ...
 */
PUCHAR push_data(netbuf_ctrl_block_t *netbuf_cb, uint16 len)
{
	PUCHAR status;
	struct sk_buff *skb = (struct sk_buff *)netbuf_cb->pkt_addr;
	...
	if( len > skb_headroom(skb))
	{
		ONEBOX_DEBUG(ONEBOX_ZONE_ERROR,(TEXT 
		             ("%s: Not Enough Head room: headroom = %d, len = %d\n"),__func__, skb_headroom(skb), len));
		return NULL;
	}
...
netbuf_ctrl_block_t* dequeue_pkt(void *addr)
{
	...
	netbuf_cb = (netbuf_ctrl_block_t *)rcvskb->cb;
	return netbuf_cb;
}
...
{
	/* FIXME: works for interface units {0..9}. Supported range for
...
void indicate_hal_pkt_to_os(void *dev, netbuf_ctrl_block_t *netbuf_cb_t)
{
	...
	{
		printk("<<<<< Indicating EAPOL Pkt to OS and  seq_no %d >>>>>\n", (((*(uint16 *)&netbuf_cb_t->data[5]) >> 4) & 0xfff));
	}
 ...
 */
void
initialize_timer(struct timer_list *timer, uint32 data, void *function, unsigned long timeout)
{
