...
static ONEBOX_STATUS zigb_tx_pkt(PONEBOX_ADAPTER adapter, uint8 *addr, uint8 len)
{
	...
	adapter->osi_zigb_ops->onebox_schedule_pkt_for_tx(adapter);
#endif
	adapter->osi_zigb_ops->onebox_send_pkt(adapter, netbuf_cb);
	return status;
}

static int zigb_recv_pkt(PONEBOX_ADAPTER adapter, uint8 *msg, uint16 *len)
{
	netbuf_ctrl_block_t *netbuf_cb;

	if(adapter->os_intf_ops->onebox_netbuf_queue_len(&adapter->zigb_rx_queue))
	{
		netbuf_cb = adapter->os_intf_ops->onebox_dequeue_pkt((void *)&adapter->zigb_rx_queue);
		if(netbuf_cb == NULL)
		{
...
int zigb_ioctl(struct net_device *dev,struct ifreq *ifr, int cmd)
{
	...
	switch(cmd)
	{
		case ONEBOX_ZIGB_SEND:
		{
			...
			status = zigb_tx_pkt(adapter, msg, len);
			return status;
		}
		case ONEBOX_ZIGB_RECV:
		{

			status = zigb_recv_pkt(adapter, msg, &recv_len);
