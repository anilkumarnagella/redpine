...
int zigb_main(uint8_t skip_card_ready)
{
  ...
  if (skip_card_ready) {
#ifdef ENABLE_DEBUG_PRINTS    
    RSI_DPRINT(RSI_PL1,"Skipping card ready \n");
#endif    
  }
      ...
      switch (app_cb_ptr->fsm_state)
      {
...
#ifdef ENABLE_DEBUG_PRINTS    
              RSI_DPRINT(RSI_PL1,"Setting simple desc \n");
#endif              
        ...
        case FSM_INIT_ENDDEV_ROUTER:
        {
          ...
          if (cmd_type == ZIGBEESETSIMPLEDESCRIPTOR || cmd_type == ZIGBEESTACKISUP)
          {
            rsi_status = resp->uCmdRspPayLoad.SetSimpleDescResp.status; 
            if( rsi_status == RSI_ZB_FAIL) {
              RSI_DPRINT(RSI_PL1,"Unable to Set Pre configured Link key \n");
              return RSI_ZB_FAIL;
            }
  ...
  while (1)
  {
    ...
    else if(matchDescReqSent)
    {
      if (app_info->status_var.matchDescRspStatus)
      {
        ...
        {
          /*FIXME Add user defined timeout in secs based on processor speed
